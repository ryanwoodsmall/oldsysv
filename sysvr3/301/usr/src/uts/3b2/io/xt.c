...
int     dev, flag;
{
	...
	if (lp->l.open & bit) {
		...
		lp->l.open |= bit;
		xtlinit(&lp->chans[chan], chan, LINK(dev), lp->l.old);
		if (flag & FEXCL)
			lp->l.xopen |= bit;
	}
 ...
 */
	...
	case JTRUN:
		...
		register int c, l, nc = 1;

		do {
			if (nc > TTYHOG)
				u.u_error = EINVAL;
			if ((c = fubyte((caddr_t)arg++)) < 0)
				u.u_error = EFAULT;
			if (u.u_error) 
				return;
			nc++;
	...
	case XTIOCLINK:
	case HXTIOCLINK:
		if ((nchans = ((struct xtioclm *) & arg)->nchans) > MAXPCHAN
		    || nchans < 1) {
			u.u_error = EINVAL;
			break;
		}
		if ((fp = (struct file *)getf(((struct xtioclm *) &arg)->fd))
		    == (struct file *)0)
			break;                  /* EBADF! */
		tdev = fp->f_inode->i_rdev;
		if ((fp->f_inode->i_ftype) != IFCHR
		    || (tp = cdevsw[major(tdev)].d_ttys) == (struct tty *)0) {
			u.u_error = ENOTTY;
			break;
		}
		...
		xtlinit(&lp->chans[0], 0, LINK(dev), lp->l.old);
		oldpri = spltty();
...
#if XTRACE == 1
	case XTIOCTRACE:
		if (copyout((caddr_t) &lp->l.trace, (caddr_t)arg, sizeof(struct Tbuf))) {
			u.u_error = EFAULT;
			break;
		}
	...
	case XTIOCSTATS:
		u.u_rtime = lbolt;
		if (copyout((caddr_t)lp->l.stats, (caddr_t)arg, sizeof(lp->l.stats)))
			u.u_error = EFAULT;
		break;
#endif
...
#if XTDATA == 1
	case XTIOCDATA:
		if (copyout((caddr_t)lp, (caddr_t)arg, (sizeof(struct Link) + sizeof(struct Channel) * (lp->l.nchans - 1))))
			u.u_error = EFAULT;
		break;
#endif
	case JWINSIZE:
		if (copyout((caddr_t) & lp->chans[CHAN(dev)].winsize, (caddr_t)arg, sizeof(struct jwinsize)))
			u.u_error = EFAULT;
		break;
	...
	case JAGENT:
		 {
			...
			bp->c_data[0] = (char)JAGENT;
			bp->c_data[1] = (char)n;
			if (copyin((caddr_t)lp->l.agent.desc.src, &bp->c_data[2], n)) {
				u.u_error = EFAULT;
				putcf(bp);
				goto agentout;
			}
			bp->c_last = n + 2;
			sps = spltty();
			putcb (bp, &tp->t_outq); /* runs at spl6 */
			xtvtproc (tp, T_OUTPUT); /* runs at spltty */

			if (sleep(&lp->l.agent.desc.dest, (TTOPRI | PCATCH))) {
				/* we were kicked out */
				u.u_error = EINTR;
				splx(sps);
			} else {
	...
	if ((count = lp->l.lihiwat - lp->l.line->t_outq.c_cc) >= sizeof(struct Packet)
		...
		for (psp = pcp->nextpkt, i = NPCBUFS ; i--;) {
			if (psp->size == 0
				...
				makepkt(tp, pcp, psp);
			if (psp->state == PX_READY && count >= psp->size) {
				...
				if (!lp->l.hex) {
#ifndef vax
					bcopy((char *) & psp->pkt, cb->c_data, (int)psp->size);
#else
...
sendnchar:
		...
		case JAGENT & 0xFF:
			...
			bcopy((char *)cp, lp->l.agent.retcb->c_data, (int)pkp->pkt.HEADER_DSIZE);
			wakeup (&lp->l.agent.desc.dest);
			return 1;
