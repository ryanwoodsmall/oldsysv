...
#ident	"@(#)kern-port:nudnix/que.c	10.1"
/*****
...
QCTRL	*qctrl;		/* queue control element */
QITEM	*qiptr;		/* queue item pointer */
{
...
QCTRL	*qctrl;		/* queue control element */
{
	...
	qiptr = qctrl->qc_head;
	if (qiptr != NULL)
		qctrl->qc_head = qctrl->qc_head->qi_next;
	return qiptr;
}
