...
char *dir;
{
	...
	char *dirname;
	struct lbuf *read_dir();

	if ((dirname = (char *)malloc(strlen(dir)+DIRSIZ+2))==NULL)
		error(errmalloc, NLSMEM);
	if ((lp = read_dir(dir)) == (struct lbuf *)NULL)
		return;
	for (; lp->link; lp = lp->link) {
		sprintf(dirname, "%s/%s", dir, lp->lname);
		if (isdir(dirname))
			process_dir(dirname);
	}
}
...
char *dir;
{
	...
	if ((first = (struct lbuf *)malloc(sizeof(struct lbuf))) == NULL)
		error(errmalloc, NLSMEM);
...
char *s;
{
	...
	if ((copy = (char *)malloc(strlen(s) + 5)) == NULL)
		error(errmalloc, NLSMEM);
	strcpy(copy, s);
	if (mknod(copy, 0040777, 0) != 0) {
		fprintf(stderr, "%s: could not create %s, errno = %d\n",arg0, copy, errno);
		Errno = NLSCREAT;
		return(0);
	}
	if (chown(copy, nlsuid, nlsgid) < 0) {
		fprintf(stderr,errchown,arg0, copy, errno);
		Errno = NLSCHOWN;
		return(0);
	}
	strcat(copy, "/.");
	if (link(s, copy) != 0) {
		fprintf(stderr, "%s: could not link %s, errno = %d\n",arg0,copy,errno);
		Errno = NLSLINK;
		(void) unlink(s);
		return(0);
	}
