...
/*	static char  ID[] = "@(#)optim:3b/w2opt.c	1.16";	*/
...
**/
	...
	&&  (
		( (f = samereg(pf->op2,pl->op1)) && ! usesreg(pl->op2,pl->op1))
...
if(dst(pf,&ptr,&num)) {
	...
	&&  strcmp(pl->op2,"&0") == 0
	&&  strcmp(ptr,pl->op1) == 0
	&&  pf->userdata->oflags & ARITH
	&&  stype(cop1) == stype(cop2)
	&&  (!iscclive(pl) || !(pl->forw->userdata->oflags & UNSIGN))
	)
    {
...
*/
    if (cop1 == MOVW && cop2 == MOVW && !usesvar(pf->op1,pf->op2))
    {
...
*/
	...
	f = ( isdead(reg,pl) || strcmp(ptr,reg) == 0 );
	if ( (! f) && ((iscclive(pl) && !isswitch(nextinst))
#ifdef CALLCMPW
					|| isret(nextinst)
#endif
	    ))
...
*/
    ...
    {
	wchange();
	makelive((pl->op1 = pf->op1),pf); /* propagate liveness of R */
	return(true);
    }
...
    {
	...
	if (src1 >= dst1)
	{
	    ...
	    makelive(pl->op1,pf);	/* make register live after pl */
	    return(true);
	}
	...
	if (src1 >= dst2)
	{
	    ...
	    pl->op1 = pf->op1;		/* propagate register/nibble use */
	    makelive(pl->op1,pf);	/* make this register live here */
	    return(true);
	}
    }
...
*/
	...
	&&  ! iscclive(pl)
#ifdef CALLCMPW
	&&  (!isret(pl->forw) || (strcmp(pl->op2,"%r0") != 0))
#endif
	)
    {
	wchange();
