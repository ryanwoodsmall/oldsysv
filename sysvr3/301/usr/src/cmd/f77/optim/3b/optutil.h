...
/*	static char  ID[] = "@(#) optutil.h: 1.5 1/20/84";	*/
...
extern int dst();		/* returns int and sets a character pointer */
...
/* macro to transfer liveness of CCODES */

#define TRCCLIVE(p,q)	{p->nlive |= (q->nlive & CCODES);}
#define KILLCC(p)	{p->nlive &= ~CCODES;}
#define BORNCC(p)	{p->nlive |= CCODES;}
