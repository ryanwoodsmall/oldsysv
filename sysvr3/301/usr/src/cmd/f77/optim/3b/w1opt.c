...
/*	static char  ID[] = "@(#)optim:3b/w1opt.c	1.9";	*/
					...
					*/
{
    ...
    int num;				/* return code from 'dst' */
    char *ptr;				/* pointer to destination operand */
/* eliminate dead code:
**
...
{
    if (
	    isdead(ptr,pf) 		/* we don't really care what the op
					** is
					*/
	&&  ! isbr(pf)			/* but some branches set variables
					** and jump:  keep them
					*/
	&& !side(pf)			/* and doesn't have a side effect */
					/* and either doesn't set the 
					 * condition code or, if it does,
					 * the instruction following is not
...
**	umulw2	&0,O1,O2	->	movw &0,O2
**	andw3	&0,O1,O2	->	movw &0,O2
**
**	mulw2	&-1,O		->	mcomw O,O
**	divw2	&-1,O		->	mcomw O,O
**	xorw2	&-1,O		->	mcomw O,O
**	mulw3	&-1,O1,O2	->	mcomw O1,O2
**	divw3	&-1,O1,O2	->	mcomw O1,O2
...
*/
	...
	case 0:				/* handle all instructions with &0
	    ...
	    case LLSW2:
	    case LRSW2:
		ultype = UA_DEL;	/* will delete all of these */
		break;
...
if (dst(pf,&ptr,&num))
{
	...
	case UA_MOVZ:			/* change to move zero to operand */
	    ...
	    pf->op2 = ptr;		/* second is ultimate destination */
	...
	case UA_MNEG:			/* change to move negated */
	    ...
	    retval = true;		/* made a change */
	    break;
/* For this case we must be careful:  if the following instruction is a
...
*/
    ...
    if (istriadic(pf,&opn,&opst) && strcmp(pf->op2,pf->op3) == 0)
...
    {
	...
	if(cop == RET &&
		/* !isdead("%r0",pf->back) && */
		(pf->back->op != CMPW || strcmp(pf->back->ops[1],"%r0") != 0) &&
		pf->back->op != CALL &&
		(!dst(pf->back,&ptr,&num) ||
		strcmp(ptr,"%r0") != 0)) {

			NODE * newnode = insert(pf->back);

			wchange();
			chgop(newnode,CMPW,"cmpw");
			newnode->op1 = "%r0";
			newnode->op2 = "&0";
			newnode->nlive = pf->nlive;
			/*BORNCC(newnode) */
			return(true);
	}
#endif
