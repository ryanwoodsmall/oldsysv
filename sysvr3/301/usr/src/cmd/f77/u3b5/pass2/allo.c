...
#ident	"@(#)comp:common/allo.c	1.6.3.1"
/*	@(#) allo.c: 1.6.3.1 1/14/86	*/
...
freereg( p, n ) NODE *p; {
	...
	if( n&NAMASK ){
	/* Logical operations have type int and represent an implicit conversion.
	** If the operands are float/double, however, temporary registers must be
	** paired, in case the intermediates requested are doubles.
	*/
		if ( logop( p->in.op ))
		{
		    TWORD ltype = p->in.left->tn.type;
		    TWORD rtype = (optype( p->in.op ) == BITYPE ?
					p->in.right->tn.type :
					FLOAT);	/* right type, FLOAT default */
		    /* for C, pass down either a FLOAT or DOUBLE and assume
		    ** that szty in usable() will say that needs two regs.  For
		    ** f77, pass down FLOAT only if both are FLOAT.
		    ** The net result is we always pass down the left node.
		    */

		    if (   ltype == FLOAT || ltype == DOUBLE
			|| rtype == FLOAT || rtype == DOUBLE)
			p = p->in.left;		/* for purposes of register alloc */
		}
