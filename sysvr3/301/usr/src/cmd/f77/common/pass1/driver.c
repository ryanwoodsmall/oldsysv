...
#ident	"@(#)f77:common/pass1/driver.c	1.12.1.1"
/*	@(#)f77:common/pass1/driver.c	1.12.1.1	*/
...
#					endif
		...
		case 'g':
			if (optimflag) {
				fprintf(diagfile,
					"'g' flag ignored when 'O' flag set.\n");
				break;
				}
			gflg = 1;
			goto copyfflag;
		...
		case 'O':
			if (gflg) {
				fprintf(diagfile,
					"'O' flag ignored when 'g' flag set.\n");
				if (isdigit(s[1])) ++s;
				break;
				}
...
if ( argc == 0 )
   {
	fprintf(stderr,"f77: Zero arguments given or no files to compile\n");
	exit(1);
   }
for(i = 0 ; i<argc ; ++i)
