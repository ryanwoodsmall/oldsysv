...
#ident	"@(#)f77:common/pass1/putpcc.c	1.7.1.2"
/*	@(#)f77:common/pass1/putpcc.c	1.7.1.2	*/
...
*/
	...
	p2pi("\t.data\n#SWBEG\n\t.align\t%d",ALIADDR);
	putlabel(labarray);
	p2pi("\t.word\t.L%ld", (ftnint) (skiplabel = newlabel()) );
	for(i = 0 ; i < nlab ; ++i)
		if( labs[i] )
			p2pi("\t.word\t.L%ld", (ftnint)(labs[i]->labelno) );
	p2pass("#SWEND\n\t.text");
	prcmgoto(index, nlab, skiplabel, labarray);
	putlabel(skiplabel);
#endif
