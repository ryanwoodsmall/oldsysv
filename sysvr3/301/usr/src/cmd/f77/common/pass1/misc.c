...
#ident	"@(#)f77:common/pass1/misc.c	1.5.1.1"
/*	@(#)f77:common/pass1/misc.c	1.5.1.1	*/
#include "defs"


/* The following are declarations for the new storage allocation
 * functions, ckalloc and ckfree. The original ckalloc has been
 * commented out. There are 5 #defines in "defs" which are needed:
 ...
 *
 *	anode - Initial sequential list of nodes.
 *	snode - Pointer to sequential list of nodes.
 *	enode - End of sequential list of nodes.
 *	lnode - Pointer to linked list of nodes.
 */

static ptr
    anode[INITSIZE][NODEWORD],
    snode = (ptr) anode,
    enode = (ptr) anode + (INITSIZE * NODEWORD),
    lnode = NULL;
...
/* New storage allocation functions. Requests for nodes of size <= NODEBYTE
 * will be allocated from a list of nodes. The allocated node will be of
 * size NODEBYTE. The list is initially sequential (array anode).
 * As nodes are freed up (by calls to ckfree) they are placed on a
 * linked list. When the sequential list is depleted, nodes are taken
 * from the linked list. When the linked list is depleted, more memory
 * is obtained via a call to calloc. This memory is treated as a
 * sequential list of nodes. All allocations of size > NODEBYTE are
 * performed by calling calloc directly.
 * When changing the size of the nodes (i.e. changing the values of
 * NODEBYTE and NODEWORD), the code which zeros out the node 
 * (i.e. p[0] = p[1] = ... = 0) must be adjusted for the new size node.
 */
...
register int n;
{
	...
	if (n > NODEBYTE) {	/* call calloc for nodes > than NODEBYTE */
		if (p = (ptr) (calloc(1, (unsigned) n)) )
			return(p);
		fatal("out of memory");
		}
	else if (snode) {	/* if nodes on sequential list */
		p = snode;
		snode += NODEWORD;
		if (snode == enode)
			snode = NULL;
	...
	else if (lnode) {	/* if nodes on linked list */
		p = lnode;
		lnode = (ptr) lnode[0];
		p[0] = p[1] = p[2] = p[3] = p[4] = p[5] = 0;
		return(p);
		}
	else {		/* if nodes on seq. and linked lists depleted */
		if (p = (ptr) calloc(ALLOSIZE, NODEBYTE)) {
			snode = (ALLOSIZE == 1) ? NULL : p + NODEWORD;
			enode = p + (ALLOSIZE * NODEWORD);
			return(p);
			}
		fatal("out of memory");
		}
