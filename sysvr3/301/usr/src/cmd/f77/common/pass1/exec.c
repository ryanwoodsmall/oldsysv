...
#ident	"@(#)f77:common/pass1/exec.c	1.6.1.1"
/*	@(#)f77:common/pass1/exec.c	1.6.1.1	*/
#include "defs"
...
if(lp->tag != TPRIM)
	...
	err("redefinition of do variable");
else
	{
	if(parstate < INDATA)
		enddcl();
...
if( ISCONST(ctlstack->domax) && ISCONST(DOINIT) && ctlstack->dostepsign!=VARSTEP)
	...
	if( onetripflag )
		...
		q = mkexpr(OPMINUS, cpexpr(ctlstack->domax), cpexpr(DOINIT));
