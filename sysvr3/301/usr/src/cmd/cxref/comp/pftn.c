...
# endif
	...
	if ((class == AUTO) && ISARY(type) && (dimtab[q->fn.cdim] <= 0))
...
#ifdef LOOKUP
	fprintf(lookfp, "define	%s	%c	%c	%c	%c	%d\n",
	    p->sname,(p->sflags & SMOS) ? 'y':'n', (p->sflags & STAG) ? 'y':'n',
	    (p->sflags & SHIDDEN) ? 'y':'n', (p->sflags & SHIDDEN) ? 'y':'n',
	    p->sflags & ~(SMOS|STAG|SHIDES|SHIDDEN));
#endif
...
# endif
	...
	cendarg();
#ifdef IN_MACRO		/* sorry--but this crap can't get output */
	if (curclass != INLINE)
#endif
	{
	locctr(PROG);
...
# endif
	...
	for( i = oparam+4;  i< paramno; ++i ){
		j=paramstk[i];
		if( j>= SYMTSZ ) cerror( "gummy structure member" );
		if (j < 0)  {
			/* special case of field w/o name */
			SETOFF( al, -j);
			continue;
		}
		dstash(j);
...
*/
	if (curclass == INLINE)
	{
	    if ( strlen(inlargs) + strlen(stab[idn].sname) > BUFSIZ -2 )
		cerror( " out of room for asm argument storage ");
#ifdef LINT
/* Don't complain about arguments not being used by asm function */
	    stab[idn].sflags |= (SSET|SREF|SMOS);
	    stab[idn].suse = -5;
#endif
	    strcat( inlargs, stab[idn].sname);
	    strcat( inlargs, "#");
	    ninlargs++;
	}
#endif
...
int ilocctr = DATA;  /* location counter for current initialization */
...
falloc( p, w, new, pty )  register struct symtab *p; NODE *pty; {
	...
	if( w == 0 ){ /* align only */
		SETOFF( strucoff, al );
		/* special value for param stack so dclstruct knows
		   about aligning to types of fields without names */
		if (new < 0)psave (-al);
		/* "zero size field" */
...
# endif
#ifdef LOOKUP
	fprintf(lookfp, "lookup	%s	%c	%c	%c	%c	%d\n",
	    name, (s & SMOS) ? 'y' : 'n', (s & STAG) ? 'y' : 'n',
	    (s & SHIDDEN) ? 'y' : 'n', (s & SHIDES) ? 'y' : 'n',
	    s & ~(SMOS|STAG|SHIDDEN|SHIDES));
#endif
...
# endif
		    ...
		    /* NOTICE: not all fields zeroed. innocuous garbage left */
			p->stype = TNULL;
			p->sclass = SNULL;
			rehash = 1;
			goto next;
