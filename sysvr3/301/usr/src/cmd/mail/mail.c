...
char	**argv;
{
	...
	*/
	...
	if ((my_name == NULL) || (strlen(my_name) == 0)) my_name = getpwuid(geteuid())->pw_name;
...
/*
	...
	sendto is used as work area, and if forwarding then
	sendto is set to the list of users
...
char *s;
{
	...
	fread(sendto, (unsigned)(sizeof(frwrd) - 1), 1, fd);
	if (strncmp(sendto, frwrd, sizeof(frwrd) - 1) == SAME) {
		fgets(sendto, sizeof(sendto), fd);
		fclose(fd);
		return(TRUE);
	}
	sendto[0] = '\0';	/* lets be nice */
	fclose(fd);
	return(FALSE);
}
...
char	**arglst;
{
	...
	*/
	...
	while ((c = getopt(argcnt, arglst, "f:hropqestwF:")) != EOF) switch(c) {
		/*
			do not print mail
 		*/
		case 'e':
