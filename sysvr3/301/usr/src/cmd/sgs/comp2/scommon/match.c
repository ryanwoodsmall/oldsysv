...
# endif
		...
		if( q->rshape ){
			if (q->needs & RMATCH)	/* match exactly? */
			    rightshape =
				mexact(q->rshape, (q->lshape ? r : p), 0, 0);
			else
			    rightshape=mspine(
				q->rshape, (q->lshape)?r:p, 0, 
		...
		if( q->lshape ){
			if (q->needs & LMATCH)	/* exact match */
			    ...
			    */
				...
				leftshape = rtspine( q->lshape, p );
			}
			else leftshape = mspine( q->lshape, l, lflg, leftres, 0);
