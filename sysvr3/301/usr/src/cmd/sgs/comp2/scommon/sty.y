...
chkfunny()
{
    ...
    int hasreg = -1;			/* template # with REG shape */
	...
	ophd[GENLAB] = optl[GENLAB] = hasreg;
	optb[hasreg].nxop = -1;
    }
...
static RST barray[10];			/* barray[i] contains current bit vector
					** for index value i
					*/
...
RST sofar;				/* bits already generated previously */
{
    ...
    RST curbits[10];			/* current bits per level */
    RST curmask[10];			/* current OR of previous right bits */
