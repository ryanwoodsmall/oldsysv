...
%{
	...
	static char fakename[NCHNAM+1];
			/*Flag: Turn on TYCHECK to accept but ignore
			  function tycheck*/
			/*Make it a runtime flag to allow runtime 
...
data_def:	   oattributes  SM
		...
		|  oattributes fdeclarator
			{   
				/* prepare function-level register allocation
				** tables
				*/
...
type_list:      template CM type_list
		|  template
		|  template ellipsis
		|  ellipsis
		;

template:	basic_type derived_type
		;

basic_type:	TYPE
			{ $1->in.op = FREE; }
		|  TYPE TYPE
			{ $1->in.op = $2->in.op = FREE; }
		|  TYPE TYPE TYPE
			{ $1->in.op = $2->in.op = $3->in.op = FREE; }
		|  STRUCT NAME
		|  ENUM NAME
		;
...
derived_type:	/*VOID*/
		|  derived_type LB RB
		|  derived_type LB ICON RB
		|  derived_type LP RP
		|  MUL derived_type
		|  LP derived_type RP
		;

ellipsis:	STROP STROP STROP
		{
			/*the dots cause the scanner to think we are in
			  structures or FP constants. we are not.*/
		stwart = 0;
		}
		;
...
opt_type_list:	type_list
		{
			/* Only allow (but ignore) the tycheck if the 
...
name_lp:	  NAME LP
			{
				...
				/* Unless we are doing typechecking tycheck*/
				if ( !tycheck)
				{
					stwart = SEENAME;
				}
...
init_declarator:   nfdeclarator
		...
		|  fdeclarator
			{
			    if (blevel == 0 && argno != 0) {
				uerror("incomplete or misplaced function definition");
				argno = 0;
			    }
...
optsemi		:	/* empty */
			{ werror("syntax requires ; at end of struct/union decl"); }
		|  SM
		;
...
/* don't rotate loops for lint */
		...
		|  RETURN e  SM
			{   register NODE *temp;
			    ...
			    rettype = temp->in.type = DECREF(temp->in.type);
			    if(temp->in.type == VOID)
				uerror(
				"void function %s cannot return value",
					stab[idname].sname);
			    /* remember return-type info */
			    s = temp->fn.csiz;
			    d = temp->fn.cdim;
 			    temp->tn.op = RNODE;

			    /* (Sleaze):  to detect mismatched types and get a
			    ** message that refers to RETURN, pretend the OP
			    ** is RETURN, which we will later change to ASSIGN
			    */
			    temp = buildtree( RETURN, temp, $2 );
			    if (temp->in.op == RETURN) {
				NODE * right = temp->in.right;

				/* re-build the tree with ASSIGN and new types */
				temp->in.op = FREE;
				temp = temp->in.left;	/* RNODE */
				/* avoid cascading messages:  no VOID on left */
				if ( rettype == VOID )
				    rettype = INT;
				temp->in.type = indtype( rettype );
				right = makety( right, rettype, d, s );
				temp = buildtree( ASSIGN, temp, right );
				/* clocal(), in case looking for (= RNODE any) */
				temp = clocal(temp);
			    }
...
switchpart:	   SWITCH  LP  e  RP
			{   register NODE *temp;
				...
				uerror("switch expression must have integer type");
			    else if ( (SZLONG > SZINT) && (type == LONG || type == ULONG) )
				werror("long in switch statement may be truncated");
...
e:		   e RELOP e
		...
		|  e CM e
			{  $2 = COMOP;
			    /* This is an unsatisfactory hack to keep the
			    ** compiler from dumping core in the event of
			    ** inappropriate string initializations, where
			    ** one of the operands could be NIL.  Fake up an
			    ** ICON so buildtree will survive; we'll get an
			    ** illegal initialization eventually.
			    */
			    if (strflg)			/* last string done in init */
			    {
				if ($1 == NIL) $1 = bcon(0);
				if ($3 == NIL) $3 = bcon(0);
			    }
...
term:		   term INCOP
		...
		|  NAME
			{  idname = $1;
				...
				uerror("label used in expression");
