...
again:
	...
	if( o == FCON )
	{
		/* for FP const converted to FLOAT/DOUBLE, okay if same
		** or larger type; for making smaller, must do the conversion
		** to float and back to double.  This may cause floating exception.
		**
		** Note that this code is very likely wrong for a cross-compilation
		** environment!
		*/
		if ( t == FLOAT || t == DOUBLE ) {	/* convert to FLOAT or DBL */
		    /* convert DOUBLE to FLOAT */
		    if ( lt != t && lt == DOUBLE ) {
			/* THIS MAY FAULT! */
			errno = 0;
			l->fpn.dval = FP_DTOFP(l->fpn.dval);
			if (errno)
			    werror("conversion of double to float is out of range");
		    }
		    goto paint;				/* paint rest of info */
		}
		...
		** round when it is output */
		...
		if (errno)
		    werror("conversion of double to long is out of range");
		l->tn.rval = NONAME;
		goto icon;
	}
...
#ifdef EXTADR
	if( o == MINUS && ISPTR(l->in.left->in.type))
		return(p);
#endif
	goto paint;
}
...
register NODE *p; 
{
	...
	*/
	...
	{
	case COMOP:
		/* patch up type if small structure subtree type has been changed */

		/* This handles a bug with  f( (x, x=y) )  in small structures,
		/* where the rhs of the comma gets changed to an interger type.
		/* Problems are not introduced by this because structure references
		/* like  (a,b).c  are disallowed, perhaps erroneously.
		/* Similar problems with short structures in ?: are not handle
		/* here at this time because of possible complications
		*/

		if ( BTYPE(p->in.type) == STRTY
		    && ( t = simpstr( r->fn.cdim, r->fn.csiz )) != STRTY )
			p->in.type = r->in.type;
		break;
...
setuleft:
	...
	case ER:
		...
		** The problem is that mixing pointer and non-pointer types at
		** this point causes 'pointer arithmetic' scaling where it is
		** not appropriate.
		*/

		/* (A op C1) op C2, where C1 and C2 are vanilla non-named ICON's */
		if( o == l->in.op && nncon(r) && nncon(l->in.right) )
		{
			/* let buildtree do the arithmetic */

			l->in.right = buildtree(o, l->in.right, r);

			p->in.op = r->tn.op = FREE;
			return( doptim( l ) );
		}
...
# endif
	...
	/* attempt to rewrite A = A op B into A op= B, being conservative
	** about the shape and type of A.
	*/

	case ASSIGN:
	    switch( l->in.op ) {	/* check left shape */
	    case REG:
	    case VAUTO:
	    case VPARAM:
	    case NAME:
	    case TEMP:
		switch( r->in.op ) {	/* check for ASG-able ops */
		case PLUS:
		case MINUS:
		case MUL:
		case DIV:
		...
		case LS:
		case RS:
		    /* check that left side of this == l */
		    {
			register NODE * rl = r->in.left;

			if (   rl->tn.op   == l->tn.op
			    && rl->tn.lval == l->tn.lval
			    && rl->tn.rval == l->tn.rval
			    && rl->tn.type == l->tn.type
			) {
#ifndef NODBG
			    if (opdebug > 1) {
				printf("rewrite ASSIGN to ASG OP:  change:");
				eprint(p);
			    }
#endif
			    /* move right side, change op, free nodes */
			    p->in.right = r->in.right;
			    p->in.op = ASG r->in.op;
			    r->in.op = rl->in.op = FREE;
#ifndef NODBG
			    if (opdebug > 1) {
				printf("to:");
				eprint(p);
	    ...
	    } /* end switch on left operand op */
	} /* end switch on top level op */
	return(p);
}
