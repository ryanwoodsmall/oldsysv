...
#ident	"@(#)strip:common/main.c	1.14"
/* UNIX HEADER */
#include	<stdio.h>
#include 	<ar.h>
...

{
...
#if UNIX
	fprintf(stderr,"usage:  %sstrip [-l] [-x] [-r] [-b] file ...\n", SGS);
#else
	fprintf( stderr, "usage: %sstrip [-l] [-x] [-r] [-b] [-s] [-f] file ...\n", SGS);
#endif
	exit(0);
    }
	...
	for (	; argc > 0; --argc, ++argv) {
		...
		if (!ISCOFF(TYPE(ldptr)) && (TYPE(ldptr) != ARTYPE)) {
			...
			while((nitems = fread(buf,sizeof(char), BUFSIZ, stripout)) != 0 ) {
				if (fwrite(buf,sizeof(char), nitems,tmp) != nitems) {
					error(*argv,"can't recreate file ",5);
					resetsig( );
					exit(1);
				}
