...
#ident	"@(#)strip:common/buildfcns.c	1.22"
/* UNIX HEADER */
#include	<stdio.h>
...

{
	...
	if (fflag || sflag || rflag || xflag || bflag || lflag)
...

{
	...
	if ((fwrite( &str_size, sizeof( long ), 1, strp1 ) != 1)
...
LDFILE	*fwdptr;
     ...
     *  if the -b is on then scoping information is also copied (.bf, .ef, .bb
     *	and .eb symbols)
