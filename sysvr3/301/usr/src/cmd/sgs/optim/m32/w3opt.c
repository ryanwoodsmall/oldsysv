...
register NODE * pl;			/* pointer to last inst. in window */
{
    ...
    long templ;				/* general use temporary */
/* *p++ improvement
**
...
*/
	    ...
	    )
	{
	    ...
	    pm->nlive |= pl->nlive;		/* preserve live/dead info */
	    pm->nlive &= (pl->nlive | ~pm->nlive);
	    pl->nlive |= (templ = uses(pm));
	    pl->nlive &= (~sets(pm) | templ);
	    return(true);
	}
    }
