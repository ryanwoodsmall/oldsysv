...
    /* sub-case 2:
    ...
    **  the same size or destination of movX larger than or equal to source.
    **  Examples:
    **
    **		movzbw	O1,R				s = 1; d = 4
...
** R|R2 in any other way in a source.  Note that dyadic instructions like
**
**	addw2 a,R
**
...
** transformation.
**
** If the second node kills R|R2 or R|R2 is dead after the second node,
...
#ifdef IMPADDR
	if ( cop2 == EXTZV )
	    destreg = 4;		/* in extract, is fourth operand */
#endif /* IMPADDR */
...
*/
	...
	if (f)				/* conditions permit deleting */
	    ...
	    exchange(pf);		/* otherwise exchange add/sub and
					...
					*/
	    pf->nlive |= pl->nlive;
	    pf->nlive &= (pl->nlive | ~pf->nlive);
	    pl->nlive |= (templ = uses(pf));
	    pl->nlive &= (~sets(pf) | templ);
	}
	return(true);
    }
noindex:
...
**	
**	(It has not been verified that the above remarks
**	apply to the 32100.  The constraint on the register is
...
*/
	...
	pf->nlive |= pl->nlive;		/* preserve live/dead info */
	pf->nlive &= (pl->nlive | ~pf->nlive);
	pl->nlive |= (templ = uses(pf));
	pl->nlive &= (~sets(pf) | templ);
	return(true);
