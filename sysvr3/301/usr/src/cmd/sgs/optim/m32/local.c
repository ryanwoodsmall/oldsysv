...
char *optbl[numops] = {	/* IS-25 plus subset of MIS opcodes (q&d fix 5/85) */
	"acjl", "acjle", "acjleu", "acjlu", "addb2",
	"addb3", "addh2", "addh3", "addw2", "addw3",
	"alsw2", "alsw3", "andb2", "andb3", "andh2",
	"andh3", "andw2", "andw3", "arsw2", "arsw3",
	"atjnzb", "atjnzh", "atjnzw", "bitw", "call",
	"cmpb", "cmph", "cmpw", "decpth", "decptw",
	"divw2", "divw3", "extzv", "faddd2", "faddd3",
	"fadds2", "fadds3", "fcmpd", "fdivd2", "fdivd3",
	"fdivs2", "fdivs3", "fmuld2", "fmuld3", "fmuls2",
	"fmuls3", "fsubd2", "fsubd3", "fsubs2", "fsubs3",
	"incpth", "incptw", "inctst", "insv", "jbc",
	"jbs", "je", "jg", "jge", "jgeu",
	"jgu", "jl", "jle", "jleu", "jlu",
	"jmp", "jne", "jneg", "jnneg", "jnpos",
	"jnz", "jpos", "jsb", "jz", "llsw2",
	"llsw3", "lrsw2", "lrsw3", "mcomb", "mcomh",
	"mcomw", "mfaddd2", "mfaddd3", "mfadds2", "mfadds3",
	"mfcmpd", "mfcmps", "mfcmptd", "mfcmpts", 
	"mfdivd2", "mfdivd3", "mfdivs2",
	"mfdivs3", "mfmuld2", "mfmuld3", "mfmuls2", "mfmuls3",
	"mfnegd2", "mfnegs2",
	"mfsubd2", "mfsubd3", "mfsubs2", "mfsubs3", "mmovdd",
	"mmovds", "mmovdu", "mmovdw", "mmovfa", "mmovsd",
	"mmovss", "mmovsu", "mmovsw", "mmovta", "mmovud",
	"mmovus", "mmovwd", "mmovws", "mnegb", "mnegh", "mnegw",
	"modw2", "modw3", "movab", "movah", "movaw",
	"movb", "movbbh", "movbbw", "movbhw", "movblb",
	"movblh", "movblw", "movd", "movdd", "movds",
	"movdw", "movh", "movsd", "movsw", "movtdw",
	"movthb", "movtsw", "movtwb", "movtwh", "movw",
	"movwd", "movws", "movzbh", "movzbw", "movzhw",
	"mulw2", "mulw3", "orb2", "orb3", "orh2",
	"orh3", "orw2", "orw3", "pushaw", "pushbb",
	"pushbh", "pushd", "pushw", "pushzb", "pushzh",
	"ret", "save", "subb2", "subb3", "subh2",
	"subh3", "subw2", "subw3", "switch", "switcht",
...
int aflag = false; /* indicates whether -a flag was entered */
...
yylex()
{
    ...
    while (linptr != NULL || 
	((linptr = fgets(line,LINELEN,stdin)) != NULL 
		&& ( !aflag || putasm(line) )))
    {
	...
	case '#':			/* check for special comments */
		...
		do {
			char * strtinst = linptr;
			while(*linptr != '\n') linptr++;
			if(strncmp(strtinst,"#ASMEND",7) == 0) {
				nums = sprintf(line+7,"%d",asmcnt++);
				saveop(0,strtinst,7+nums+1,ASMS);
				break;
			}
			*linptr++ = '\0';
			saveop(0,strtinst,linptr-strtinst,ASMS);
		} while((linptr = fgets(line,LINELEN,stdin)) != NULL 
			&& ( !aflag || putasm(line) ));
		if(linptr == NULL)
			fatal("unbalanced ASM\n",NULL);
	    }
	    else if (strncmp(s,"HARD",4) == 0) {
		s += 5;
	...
	register char *s;
{
	...
	while (*s)
	{
		...
		while(*s != ',' && *s != '\n' && *s != '#') 
			s++; /* find end of operand */
		if( *s == '#' ) { /* get return value register count */
			if( isret( lastnode ) ) rvregs = *(s + 1) - '0';
			*( s + 1 ) = '\0';
		}
		*s++ = '\0';
		saveop(opn++, t, s-t, 0);
	...
	switch (pop) {			/* dispatch on pseudo-op in text */
		...
		case DEF: {
			...
			while (true) {
				...
				if (*(s += 2) == 'e') /* endef */
					break;
/* There are three special cases for ".val".  The first is
** ".val ." .  Since we're going to move the .def line, we need
** to replace the '.' with a real label (which we make a hard label)
** first.  The second special case is ".val .Lxxx" for a local label.
...
** line.  The third case is "val .Lxxx" for a local static.
** Since the label for this case is in .data, it does not get
** removed by the optimizer, and no action is needed.
*/
				else if (*s++ == 'v') { /* .val */
					if (s[3] == '.')
					{
			...
			case 2:			
				...
				switch( rvregs ) {
				case 0: RETREG = 0x0000; break;
				case 1: RETREG = 0x0001; break;
				case 2: RETREG = 0x0003; break;
				}
#endif /* IMPRETVAL */
				if( !( asmflag && aflag ) ) {
#ifdef IMPREGAL
				...
				if( aflag ) asmchk();
				asmflag = false;
				rvregs = 2; /* re-init ret value reg cnt */
				init();
				break;
			case 3:			/* .val .Lxxx */
				break;		/* ignore line */
...
yysflgs( p ) char *p; { /* parse flags with sub fields */
	...
	case 'K':
		...
		fprintf(stderr,"Optimizer: K suboption missing\n");
		return( p );
#ifdef IMPIL
	case 'y':
...
lookup(op) char *op; { /* look up op code ordinal */
	...
	static unsigned char ocode[numops] = {
		...
		ADDB3, ADDH2, ADDH3, ADDW2, ADDW3,
		ALSW2, ALSW3, ANDB2, ANDB3, ANDH2,
		ANDH3, ANDW2, ANDW3, ARSW2, ARSW3, ATJNZB,
		ATJNZH, ATJNZW, BITW, CALL, CMPB,
		CMPH, CMPW, DECPTH, DECPTW, DIVW2,
		DIVW3, EXTZV, FADDD2, FADDD3, FADDS2,
		FADDS3, FCMPD, FDIVD2, FDIVD3, FDIVS2,
		FDIVS3, FMULD2, FMULD3, FMULS2, FMULS3,
		FSUBD2, FSUBD3, FSUBS2, FSUBS3, INCPTH,
		INCPTW, INCTST, INSV, JBC, JBS,
		JE, JG, JGE, JGEU, JGU,
		JL, JLE, JLEU, JLU, JMP,
		JNE, JNEG, JNNEG, JNPOS, JNZ,
		JPOS, JSB, JZ, LLSW2, LLSW3,
		LRSW2, LRSW3, MCOMB, MCOMH, MCOMW,
		MFADDD2, MFADDD3, MFADDS2, MFADDS3, MFCMPD,
		MFCMPS, MFCMPTD, MFCMPTS,
		MFDIVD2, MFDIVD3, MFDIVS2, MFDIVS3,
		MFMULD2, MFMULD3, MFMULS2, MFMULS3, 
		MFNEGD2, MFNEGS2, MFSUBD2,
		MFSUBD3, MFSUBS2, MFSUBS3, MMOVDD, MMOVDS,
		MMOVDU, MMOVDW, MMOVFA, MMOVSD, MMOVSS,
		MMOVSU, MMOVSW, MMOVTA, MMOVUD, MMOVUS,
		MMOVWD, MMOVWS, 
		MNEGB, MNEGH, MNEGW, MODW2,
		MODW3, MOVAB, MOVAH, MOVAW, MOVB,
		MOVBBH, MOVBBW, MOVBHW, MOVBLB, MOVBLH,
		MOVBLW, MOVD, MOVDD, MOVDS, MOVDW,
		MOVH, MOVSD, MOVSW, MOVTDW, MOVTHB,
		MOVTSW, MOVTWB, MOVTWH, MOVW, MOVWD,
		MOVWS, MOVZBH, MOVZBW, MOVZHW, MULW2,
		MULW3, ORB2, ORB3, ORH2, ORH3,
		ORW2, ORW3, PUSHAW, PUSHBB, PUSHBH,
		PUSHD, PUSHW, PUSHZB, PUSHZH, RET,
		SAVE, SUBB2, SUBB3, SUBH2, SUBH3,
		SUBW2, SUBW3, SWITCH, SWITCHT,
		UDIVW2, UDIVW3, UMODW2, UMODW3, UMULW2,
		UMULW3, XORB2, XORB3, XORH2, XORH3,
		XORW2, XORW3
		};
...
dst(p) NODE *p; { /* return pointer to dst operand string */
	...
	switch (p->op) {
	    ...
	    case MOVD:
	    case MFADDS2: case MFADDD2: case MFSUBS2: case MFSUBD2: case MFMULS2:
	    case MFMULD2: case MFDIVS2: case MFDIVD2: case MMOVSS: case MMOVSD:
	    case MMOVSW: case MMOVSU: case MMOVDS: case MMOVDD: case MMOVDW:
	    case MMOVDU: case MMOVWS: case MMOVWD: case MMOVUS: case MMOVUD:
	    case MFNEGD2: case MFNEGS2:
		return(p->op2);
	    ...
	    case MFADDS3: case MFADDD3: case MFSUBS3: case MFSUBD3: case MFMULS3:
	    case MFMULD3: case MFDIVS3: case MFDIVD3:
		return(p->op3);
...
stype(cop) int cop; { /* return implied type of op code */
	...
	switch (cop) {
	    ...
	    default:
		return (4);
	    case MFADDD2: case MFADDD3: case MFSUBD2: case MFSUBD3: case MFMULD2:
	    case MFMULD3: case MFDIVD2: case MFDIVD3: case MMOVSD: case MMOVWD:
	    case MMOVUD: case MMOVDD: case MFCMPD:
	    case MFNEGD2: case MFCMPTD:
...
uses(p) NODE *p; { /* set register use bits */
	...
	switch (p->op) {
	    ...
	    case MNEGW: case MOVWS: case MOVTSW: case MOVSW:
	    case MMOVSS: case MMOVSW: case MMOVSU: case MMOVWS: case MMOVUS:
	    case MFNEGS2:
	    ...
	    case SUBH3:  case SUBW3: case MULW3:  case UMULW3: case DIVW3:
	    case UDIVW3: case MODW3: case UMODW3: case ALSW3:  case ARSW3:
	    case MFADDS3: case MFSUBS3: case MFMULS3: case MFDIVS3:
	    ...
	    case FADDD2: case FSUBD2: case FMULD2: case FDIVD2:
	    case MFMULD2: case MFDIVD2: case MFADDD2: case MFSUBD2: 
	    case FCMPD:
	    case MFCMPD:
	    case MFCMPTD:
	...
	while (*cp != '\0') {			/*	'\0'	dead	dead	*/
		if (*cp == '%') {
			...
			switch (*cp) {
			    ...
			    case 'f':
				cp++;
				if (*cp == 'p') set = set | 1 << 10;
				break;
			    case 's':
				cp++;
				if (*cp == 'p') set = set | 1 << 11;
				break;
			    }
			}
		cp++;
...
/* Register doubles set two registers at a time:  r and r+1 */
	    ...
	    case MFDIVD3: case MFMULD3: case MFSUBD3: case MFADDD3:
	    case MMOVUD: case MMOVWD: case MMOVSD:
	    case MFMULD2: case MFDIVD2: case MFADDD2: case MFSUBD2: case MMOVDD:
	    case MFNEGD2:
		dblflag = true;
