...

{
	...
	/* set doubles flag */
	case 'N': nodblflg = 1; return; /* there are no doubles */
	case 'R': return; /* absorb RVREGS messages */
	}
...

{
	...
	/* check for doubles or asms */
	if( nodblflg == 0 || asmflag ) {
		vars = 0;
		return( numnreg );
	}
	...
	if( iascut < 0 ) { /* if no payoff for this optimization */
		nodblflg = 0;  /* reinitialize for global register dubble flag -- bkc 7/30/84 */
		vars = 0;
		return( numnreg );	
	}
...
ravassign()
{
	...
	for( ivar = 0; ivar < vars; ivar++ ) {
		...
		/* check for being MIS operand */
		if (misoper(ph->hiname)) continue;
...
register char *name;
{
	...
	for (ALLN(pn)) {
		switch(pn->op) {
		case MMOVWS:		/* all MIS instructions with int as src */
		case MMOVWD:
		case MMOVUS:
		case MMOVUD:
		case MMOVFA:
			if (strcmp(name, pn->op1) == 0) return(1);
			break;
		case MMOVSW:		/* all MIS instructions with int as dst */
		case MMOVDW:
		case MMOVSU:
		case MMOVDU:
		case MMOVTA:
			if (strcmp(name, pn->op2) == 0) return(1);
			break;
		default:
			continue;
		}
...

#ifdef IMPRETREM
/* removal of extra returns */

void
rer()
{
	register NODE *pn, *p, *r, *rr;
	NODE *pret;

	/* check for unused returns */
	pret = NULL;
	for( ALLN( pn ) ) {
		if( pn->op != RET ) continue;
		for( p = pn->back; p != 0; p = p->back ) {
			if( isuncbr( p ) ) {
				for( r = p->forw; r != pn->forw; ) {
					rr = r;
					r = r->forw;
					DELNODE( rr );
					if( islabel( rr ) ) {
...

/* check whether label in the string pointed to is used in function */
chklabel( cp )
char *cp;
{
	NODE *pn;
...
int	intpc = -3;	/* percent limit on in-line expansion */
char	*pcdecode();
