...
fpfold() { /* fold moves following floating point operations */

	register NODE *p, *pf, *pl;
	register int cop1, cop2;
	extern boolean isdeadfp();
	extern void rainsld();
	extern void raremld();

	rainsld();	/* insert pointers for live/dead analysis */
	for(ALLN( p ) ) {
		pf = p;
		pl = p->forw;
		if( pl == &ntail ) break;
		cop1 = pf->op;
		cop2 = pl->op;
/* merge fp moves back into other moves 
**
**	mmovXd O1,F	->	mmovXd O1,O2
**	mmovdd F,O2	
** or
**	mmovXs O1,F	->	mmovXs O1,O2
**	mmovss F,O2
**
**	if F dead
**
*/
    ...
    if (
  	  ( ( ( cop1 == MFNEGD2 || cop1 == MMOVDD || 
  	        cop1 == MMOVSD  || cop1 == MMOVWD ) && cop2 == MMOVDD ) ||
            ( ( cop1 == MFNEGS2 || cop1 == MMOVDS ||
  	        cop1 == MMOVSS  || cop1 == MMOVWS ) && cop2 == MMOVSS ) 
          )
	&& isreg(pl->op1)	/* if is a reg, then it is a MAU reg */
	&& strcmp(pf->op2,pl->op1) == 0
	&& isdeadfp(pl->op1,pl) /* is F dead */
       )

    {
	wchange();		/* making a change */
	pf->op2 = pl->op2;	/* copy operand pointer */
	lmrgin1(pf,pl,pf);	/* preserve line number info */
	mvlivecc(pl);		/* preserve condition code info */
	DELNODE(pl);		/* delete second node */
	continue;
    }
/* merges fp moves back into dyadics
**
**	mfopd2 O1,F	->	mfopd3 O1,F,O2
**	mmovdd F,O2		
** or
**	mfops2 O1,F	->	mfops3 O1,F,O2
**	mmovss F,O2
**
**	if F dead
**
*/
    ...
    if (
  	  ( ( ( cop1 == MFADDD2 || cop1 == MFDIVD2 || 
  	        cop1 == MFMULD2 || cop1 == MFSUBD2 ) && cop2 == MMOVDD ) ||
            ( ( cop1 == MFADDS2 || cop1 == MFDIVS2 ||
  	        cop1 == MFMULS2 || cop1 == MFSUBS2 ) && cop2 == MMOVSS ) 
          )
	&& isreg(pl->op1)	/* if is a reg, then it is a MAU reg */
	&& strcmp(pf->op2,pl->op1) == 0
	&& isdeadfp(pl->op1,pl) /* is F dead */
	...
	wchange();		/* making a change */
	switch( cop1 ) {
	case MFADDD2: chgop( pf, MFADDD3, "mfaddd3" ); break;
	case MFDIVD2: chgop( pf, MFDIVD3, "mfdivd3" ); break;
	case MFMULD2: chgop( pf, MFMULD3, "mfmuld3" ); break;
	case MFSUBD2: chgop( pf, MFSUBD3, "mfsubd3" ); break;
	case MFADDS2: chgop( pf, MFADDS3, "mfadds3" ); break;
	case MFDIVS2: chgop( pf, MFDIVS3, "mfdivs3" ); break;
	case MFMULS2: chgop( pf, MFMULS3, "mfmuls3" ); break;
	case MFSUBS2: chgop( pf, MFSUBS3, "mfsubs3" ); break;
	}
...
/* merges fp moves back into triadics
**
**	mfopd3 O1,O2,F	->	mfopd3 O1,O2,O3
**	mmovdd F,O3
** or
**	mfops3 O1,O2,F	->	mfops3 O1,O2,O3
**	mmovss F,O3
**
**	if F dead
**
*/
    ...
    if (
  	  ( ( ( cop1 == MFADDD3 || cop1 == MFDIVD3 || 
  	        cop1 == MFMULD3 || cop1 == MFSUBD3 ) && cop2 == MMOVDD ) ||
            ( ( cop1 == MFADDS3 || cop1 == MFDIVS3 ||
  	        cop1 == MFMULS3 || cop1 == MFSUBS3 ) && cop2 == MMOVSS ) 
          )
	&& isreg(pl->op1)	/* if is a reg, then it is a MAU reg */
...
/* check if floating point register is dead */

boolean
isdeadfp( reg, pn ) /* is reg dead following pn */
char *reg;
NODE *pn;
{
	NODE *q;
	static int ctr = 0;  /* unique identifier for scans */
	extern NODE *fplive();

	q = fplive( pn->forw, reg, ++ctr );
	return( q == NULL );
}
/* check for floating point register being live */

/* Returns non-NULL if register is live */
...
register char *name;	/* pointer to register name */
int ctr;		/* unique identifier for scans */
{
	NODE *qn;
	register int iop;
	register char *p;
	...
	for( ; pn != &ntail; pn = pn->forw ) {
		...
		for( iop = 1; iop < MAXOPS + 1; iop++ ) {
			...
			if( *p == *name /* for speed */
				&& strcmp( name, p ) == 0 ) {
				/* see if it's a pure destination */
				if( iop==2 )
					switch ( pn->op ) {
					case MMOVSS: case MMOVSD:
					case MMOVDS: case MMOVDD:
					case MMOVWS: case MMOVWD:
					case MMOVUS: case MMOVUD:
					case MFNEGD2: case MFNEGS2:
						return( NULL );
					}
				if( iop==3 ) 
					switch ( pn->op ) {
					case MFADDS3: case MFADDD3:
					case MFSUBS3: case MFSUBD3:
					case MFMULS3: case MFMULD3:
					case MFDIVS3: case MFDIVD3:
						return( NULL );
					}
				/* nope, it's live */
				return( pn );
	...
	if( ( qn = fplive( (NODE *) pn->opm, name, ctr ) ) != NULL ) 
		return( qn );
	if( isuncbr( pn ) ) return( NULL );
	qn = fplive( pn->forw, name, ctr );
	return( qn );
}
