...
char	**argv;
{
	...
	*argv = p1 + 1;
	...
	if (**argv == 'r')
		rflg++;
	home = getenv("HOME");
	while ((c = tmpgetopt(argc, argv,"-sp:qx",""))!= EOF) {
		switch(c) {
		...
		default:
			write(2,"usage: ed [-s] [-p string] [-x] [file]\n",43);
			exit(2);
		}
	}
	argc = argc - argind;
...
#ifndef RESEARCH
		/*
		 * NOTE: the shifting algorithm used in the next lines
