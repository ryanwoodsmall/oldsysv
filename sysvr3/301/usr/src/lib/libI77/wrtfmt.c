		...
		/* instead we know too much about stdio */
{
	...
	if(cursor > 0) {
		if(hiwater <= recpos)
			...
			if(cf->_flag & _IOLBF || !curunit->useek)		/* cdm 5-17-84 */
				...
				(void) fseek(cf, (long) (hiwater - recpos), 1);	/* cdm 5-17-84 */
			cursor -= hiwater - recpos;
			recpos = hiwater;
			for(; cursor > 0; cursor--)
				(*putn)(' ');
			...
			if(cf->_flag & _IOLBF || !curunit->useek)		/* cdm 5-17-84 */
				cf->_ptr += cursor;				/* cdm 5-17-84 */
			else							/* cdm 5-17-84 */
				(void) fseek(cf, (long)cursor, 1);		/* cdm 5-17-84 */
			recpos += cursor;
		}
	}
	if(cursor<0)
	{
		...
		if(!(cf->_flag & _IOLBF) && curunit->useek)		/* cdm 5-17-84 */
			(void)fseek( cf, (long)cursor, 1 );		/* cdm 5-17-84 */
		else if(cf->_ptr + cursor >= cf->_base)			/* cdm 5-17-84 */
			cf->_ptr += cursor;				/* cdm 5-17-84 */
		else							/* cdm 5-17-84 */
			err( elist->cierr, 110, "cannot tab left" );	/* cdm 5-17-84 */
		if(hiwater < recpos)
			hiwater = recpos;
		recpos += cursor;
		cursor=0;
...
{	char *s;
	...
	if (len == sizeof(float))
		...
		dd = p->pd;
	s=ecvt( dd,scale<0?d+scale:d,&dp,&sign);	/* cdm 3-30-84 */
