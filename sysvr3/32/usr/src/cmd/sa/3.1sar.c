...
#ident	"@(#)sa:3.1sar.c	1.2"
/*	sar.c 1.30 of 4/1/87	*/
/*
...
prtopt()
{
	...
	case 'U':
		if (nx.bpb_utilize) {
			...
			(float)(nx.bi.usr - ox.bi.usr)/tdiff * 100.0,
			(float)(nx.bi.sys - ox.bi.sys)/tdiff * 100.0,
			"",
			(float)(nx.bi.idle - ox.bi.idle)/tdiff * 100.0,
			(float)(nx.bi.syscall - ox.bi.syscall)/tdiff * HZ);
	
			ax.bi.usr += nx.bi.usr - ox.bi.usr;
			ax.bi.sys += nx.bi.sys - ox.bi.sys;
			ax.bi.idle += nx.bi.idle - ox.bi.idle;
			ax.bi.syscall += nx.bi.syscall - ox.bi.syscall;
...
prtavg()
{
	...
	case 'U':
		if (nx.bpb_utilize) {
				printf("Average  %7.0f %7.0f %7s %7.0f %7.0f\n",
				(float)ax.bi.usr/tdiff * 100.0,
				(float)ax.bi.sys/tdiff * 100.0,
				"",
				(float)ax.bi.idle/tdiff * 100.0,
				(float)ax.bi.syscall/tdiff * HZ);
		}
		break;
	case 'y':
#ifndef u370
