...
Check:		if (action && newaction != action) {
	...
	default:
		/*
		 * "optopt" is an undocumented feature of "getopt()".
		 * It appears to contain the option letter where things
		 * wrong. If it's an option we recognize, the problem
		 * must be a missing argument.
		 */
		if (optopt == '?') {
			usage ();
			exit (0);
