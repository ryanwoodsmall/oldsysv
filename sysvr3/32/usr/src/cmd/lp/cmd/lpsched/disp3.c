...
#ident	"@(#)nlp:cmd/lpsched/disp3.c	1.10"
# include	"dispatch.h"
 ...
 ** _mount() - S_MOUNT
 **
 ** _mount instead of mount to avoid conflict with mount(2)
 **/
	...
	NODE			*n;
{
	...
	send (n, R_UNMOUNT, status);
	return;
}

/**
 ** load_form() - S_LOAD_FORM
 **/
	...
	NODE			*n;
{
	...
	send (n, R_LOAD_FORM, status);
	return;
}

/**
 ** unload_form() - S_UNLOAD_FORM
 **/
	...
	NODE			*n;
{
	...
	send (n, R_UNLOAD_FORM, status);
	return;
}

/**
 ** load_printwheel() - S_LOAD_PRINTWHEEL
 **/
	...
	NODE			*n;
{
	...
	send (n, R_LOAD_PRINTWHEEL, status);
	return;
}

/**
 ** unload_printwheel() - S_UNLOAD_PRINTWHEEL
 **/
	...
	NODE			*n;
{
	...
	send (n, R_UNLOAD_PRINTWHEEL, status);
	return;
}
