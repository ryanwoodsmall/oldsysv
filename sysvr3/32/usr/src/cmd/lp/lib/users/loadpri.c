...
#ident	"@(#)nlp:lib/users/loadpri.c	1.7"
/*	Copyright (c) 1984 AT&T	*/
/*	  All Rights Reserved  	*/
...
empty:
    ...
    while (1) {
	if (!(p = fgets(line, BUFSIZ, f))) goto empty;
	p = line;
...
Error:
    ...
    if (pri_tbl.deflt == -1) pri_tbl.deflt = LEVEL_DFLT;
    if (pri_tbl.deflt_limit == -1) pri_tbl.deflt_limit = LIMIT_DFLT;
    endpwent();
    close_lpfile(f);
    return (&pri_tbl);
}
