 ...
 *	All Rights Reserved
 */
#ident	"@(#)vm.sys:src/sys/genfind.c	1.2"
 ...
 */
genfind(argc, argv, instr, outstr)
int	argc;
char	*argv[];
IOSTRUCT	*instr;
IOSTRUCT	*outstr;
...
char	*type;
{
	...
	for (i = 0; i < ott_len; ++i) {
		...
		if ((!objtype || !strcmp(objtype, typename) || !strcmp(intobj, type)) && regex(pattern, objname)) {
			...
			if (min_days < 0 || (numdays >= min_days && numdays <= max_days)) {
				...
				if (uid < 0 || filestat.st_uid == uid) {
					...
					path[length] = '\0';
					if (do_traverse)
						putastr(path_to_title(nstrcat(path, "/", objname, NULL), prefix), outstr);
					else
