 ...
 *	All Rights Reserved
 */
#ident	"@(#)vm.sys:src/sys/coproc.c	1.8"
...
extern int Fmli_pid;		/* process ID of FMLI */

int
cosend(argc, argv, instr, outstr)
int argc;
char **argv;
IOSTRUCT *instr;
IOSTRUCT *outstr;
{
	...
	osig = signal(SIGPIPE, SIG_IGN);	/* ignore interrupts */
...
int
cocreate(argc, args, instr, outstr)
int argc;
char **args;
IOSTRUCT *instr;
IOSTRUCT *outstr;
{
	...
	if (!tmp.r_path) {
		...
		sprintf(path, "/tmp/r%s.%d", tmp.id, Fmli_pid);
		tmp.r_path = strsave(path);
	}
	if (!tmp.w_path) {
		char path[PATHSIZ];

		sprintf(path, "/tmp/w%s.%d", tmp.id, Fmli_pid);
...
int
codestroy(argc, args, instr, outstr)
int argc;
char **args;
IOSTRUCT *instr;
IOSTRUCT *outstr;
...
int
cocheck(argc, argv, instr, outstr)
int argc;
char **argv;
IOSTRUCT *instr;
IOSTRUCT *outstr;
...
int
coreceive(argc, argv, instr, outstr)
int argc;
char **argv;
IOSTRUCT *instr;
IOSTRUCT *outstr;
