 ...
 *	All Rights Reserved
 */
#ident	"@(#)vm.sys:src/sys/mencmds.c	1.15"
...
int
cmd_setodi(argc, argv, instr, outstr)
int argc;
char *argv[];
IOSTRUCT	*instr;
IOSTRUCT *outstr;
...
int
cmd_getodi(argc, argv, instr, outstr)
int argc;
char *argv[];
IOSTRUCT	*instr;
IOSTRUCT *outstr;
...
int
cmd_run(argc, argv, instr, outstr)
int argc;
char *argv[];
IOSTRUCT	*instr;
IOSTRUCT *outstr;
...
int
cmd_unset(argc, argv, instr, outstr)
int argc;
char *argv[];
IOSTRUCT	*instr;
IOSTRUCT *outstr;
...
int
cmd_set(argc, argv, instr, outstr)
int argc;
char *argv[];
IOSTRUCT	*instr;
IOSTRUCT *outstr;
...
int
cmd_message(argc, argv, instr, outstr)
int argc;
char *argv[];
IOSTRUCT	*instr;
IOSTRUCT *outstr;
{
	...
	for (i = 1; argv[i]; i++) {
		if (argv[i][0] == '-') {
			...
			amessage = msg;
			strncat(msg, argv[i], MAX_MSG - 1 - strlen(msg));
			strncat(msg, " ", MAX_MSG - 1 - strlen(msg));
		}
	}
...
int
cmd_indicator(argc, argv, instr, outstr)
int argc;
char *argv[];
IOSTRUCT	*instr;
IOSTRUCT *outstr;
 ...
 */
cmd_regex(argc, argv, instr, outstr)
int argc;
char *argv[];
IOSTRUCT	*instr;
IOSTRUCT	*outstr;
...
int
cmd_getlist(argc, argv, instr, outstr)
int argc;
char *argv[];
IOSTRUCT *instr, *outstr;
{
...
int
cmd_pathconv(argc, argv, instr, outstr)
int argc;
char *argv[];
IOSTRUCT *instr, *outstr;
{
	...
	if (input && *input && *input != ' ')
		if (fullpath) {
			...
			putastr(path_to_title(input, pre), outstr);
	if (freeit)
		free(input);
	return SUCCESS;
}

cmd_reinit(argc, argv, instr, outstr)
int argc;
char *argv[];
IOSTRUCT *instr;
IOSTRUCT *outstr;
...
cmd_setcolor(argc, argv, instr, outstr)
int argc;
char *argv[];
IOSTRUCT *instr;
IOSTRUCT *outstr;
