...
#ident	"@(#)vm.oh:src/oh/fm_mn_par.c	1.9"
...
register struct attribute *attr;
{
	if (attr->flags & FREEIT) {
		if (attr->def)
			free(attr->def);
		if (attr->cur) {
			if (((attr->flags & RETS) == RET_LIST) || ((attr->flags & RETS) == RET_ARGS))
				listfree(attr->cur);
			else
				free(attr->cur);
		}
...
int no;
{
	...
	fld->attrs = (struct attribute **) array_create(sizeof(struct attribute *), no);
	for (i = 0; i < no; i++) {
		if (defaults[i].flags & MAKE_COPY) {
			p = new(struct attribute);
			*p = defaults[i];
			p->flags |= FREEIT;
		}
		else
...
** information in.
*/
parser(flags, info_or_file, sdfl, sdflsize, single, mdfl, mdflsize, multi)
int flags;
char *info_or_file;
...
struct fld **multi;
{
	...
	if (flags & INLINE) 
		...
		if ((fp = fopen(info_or_file, "r")) == NULL) {
			warn(NOPEN, info_or_file);
			if (single)
				single->attrs = NULL;
			return;
		}
	...
	while (more = eval(in, out, EV_GROUP)) {
		...
		for (i = 0; i < sdflsize; i++)
			if (strCcmp(kwd, sdfl[i].testring) == 0)
				break;
		if (i == sdflsize) {
			if (strCcmp(kwd, mdfl[0].testring) == 0)
				break;
		}
		else {
	...
	for ( ; more; more = eval(in, out, EV_GROUP)) {
		...
		for (i = 0; i < mdflsize; i++)
			if (strCcmp(kwd, mdfl[i].testring) == 0)
				break;
		if (i == 0) {		/* if its the first multi-descriptor */
...
int *ptype;
{
	...
	*pcol = VT_UNDEFINED;
	*ptype = 0;
	if (strCcmp(begrow, "current") == 0)
		*ptype = VT_COVERCUR;
	else if (strCcmp(begrow, "center") == 0)
		*ptype = VT_CENTER;
