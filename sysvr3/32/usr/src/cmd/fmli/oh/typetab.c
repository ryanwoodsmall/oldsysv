...
#ident	"@(#)vm.oh:src/oh/typetab.c	1.5"
...
static char Ott_version[] = "OTT V1.0\n";
...
bool readall;		/* read the .ott as well as cross indexing directory */
{
	...
	char name[ONAMESIZ], dname[DNAMESIZ], display[DISPSIZ],
		obtype[OTYPESIZ], odinfo[500];
	long mask, mtime;
	int hindex, val;
	struct ott_entry *entry;
	struct stat sbuf, dirsbuf;
	struct direct dir;		/* for reading in the directory structure */
	...
	if (readall) {
		Creation_size = dirsbuf.st_size/sizeof(struct direct);
		ott_init();						/* get a new current ott */
		Cur_ott->dir_mtime = dirsbuf.st_mtime;

		if (strlen(path) + Ott_len + 1 > PATHSIZ) {
#ifdef _DEBUG
		...
		if ((ottfp = fopen(ottname, "r")) != NULL &&
			...
			while (fgets(buf, BUFSIZ, ottfp) != NULL) {
				...
				_debug(stderr, ".");
#endif
				if (scanbuf(buf, name, 30) ||
					scanbuf(NULL, dname, 15) ||
					scanbuf(NULL, display, 24) ||
					scanbuf(NULL, obtype, 15) ||
					scanhex(NULL, &mask) ||
					scanhex(NULL, &mtime)) {
				...
				scanbuf(NULL, odinfo, 500);	/* odi allowed to be null */
				...
				strncpy(entry->name, name, ONAMESIZ);
				entry->name[ONAMESIZ - 1] = '\0';
	...
	if (Cur_ott->dir_mtime >= Cur_ott->ott_mtime) {
		...
		if (stray)
			...
			stray = (char *)array_create(DIRSIZ+1, Creation_size/2 + 1);
			array_ctl(stray, OTT_ENTRIES/2);
		}

		(void) fseek(dirfp,(long)(2*sizeof(struct direct)),0);	/*skip . & .. */

		while (fread(&dir, sizeof(dir), 1, dirfp) > 0) {
			if (dir.d_ino == 0)		/* file was deleted, skip it*/
				continue;
...
bool nosort;		/* don't sort if not dirty */
{
	...
	if (Cur_ott->modes & OTT_DMAIL)
		Cur_ott->numpages = (array_len(Cur_ott->parents)+5) / 6;
	else
		Cur_ott->numpages = (array_len(Cur_ott->parents)+6) / 7;
	...
	for (i = 0; i < MAX_OTT; i++) {
		...
		if ((Otts[i].modes & OTT_ACTIVE) && (strcmp(Otts[i].path, path) == 0)) {
			...
			if (stat(path, &dirsbuf) == -1)
				return(O_FAIL);
			if (strlen(Cur_ott->path) + Ott_len + 1 > PATHSIZ) {
#ifdef _DEBUG
				_debug(stderr, ".ott path too long for %s\n", Cur_ott->path);
#endif
				return(O_FAIL);
...
ott_write()
{
	...
	for (i = 0; i < size; i++ ) {
		...
		if (!(Cur_entry[i].objmask & M_DL)) {
			...
			fprintf(ottfp,"%s|%.14s|%s|%s|%lx|%lx|%.500s\n", 
				Cur_entry[i].name,
				dname,
				display,
