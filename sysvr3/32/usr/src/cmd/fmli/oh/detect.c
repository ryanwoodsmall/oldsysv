...
#ident	"@(#)vm.oh:src/oh/detect.c	1.2"
...
char stray[][DIRSIZ+1];
{
	...
	for (i = 0; i < size; i++) {
		...
		if (psize + strlen(stray[i]) + 3 > PATHSIZ)	/* 3: "/" & prefixes */
		...
		for (heur = 0; !determined && Detab[heur].objtype[0]; heur++) {
			...
			case F_DPARTS:
				...
				if (exist_heuristics(path, stray[i], Detab[heur].objtype,
					Detab[heur].defmask, Detab[heur].defodi, sbuf.st_mtime))
					determined = TRUE;
				break;
			case F_PARTS:
			...
			case F_MAGIC:
				...
				if (magic_heuristics(path, stray[i], Detab[heur].objtype,
					Detab[heur].defmask, Detab[heur].defodi, sbuf.st_mtime,
					Detab[heur].magic_offset, Detab[heur].magic_bytes))
					determined = TRUE;
				break;
			case F_SHELL:
			case F_EXEC:
...
long mtime;
{
	...
	for (i = 1; i < numparts; i++) {
		...
		if (strlen(pattern) + Pathlen + 3 > PATHSIZ)
			return(FALSE);
...
void (*info_func)();
{
	...
	/* if any part's path is > PATHSIZ, do not display it */
	if ((found[i] != -1) && (strlen(stray[found[i]]) + Pathlen + 3 > PATHSIZ))
		return(FALSE);
