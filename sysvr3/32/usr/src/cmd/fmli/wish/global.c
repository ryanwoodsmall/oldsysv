 ...
 *	All Rights Reserved
 */
#ident	"@(#)vm.wish:src/wish/global.c	1.23"
	...
	*/
	...
	case TOK_HELP:
		...
		if (Arg_count <= 1) {
			...
			if (Pending_op)
				objop("OPEN", "TEXT", "$VMSYS/OBJECTS/Text.help", Pending_op, Pending_op, NULL);
			else
		...
		if (t == TOK_OPEN)
			goto openit;
		return TOK_NOP;
	case TOK_CHECKPRINT:	/* checkprint */
		working(TRUE);
		objop(Open, Menu, "$VMSYS/OBJECTS/Menu.ckpr", NULL);
		return TOK_NOP;
	case TOK_SECURITY:	/* security */
		working(TRUE);
		if (path = path_arg()) {
			objop(Open, Form, "$VMSYS/OBJECTS/Form.sec", path,
				path_to_title(path, NULL), NULL);
			return TOK_NOP;
		}
		break;
	...
	case TOK_FIND:		/* find */
		...
		if (parse_n_in_fold(&name, &path) == SUCCESS) {
			if (name == NULL)
				...
				objop(Open, Menu, "$VMSYS/OBJECTS/Menu.find", path, name, Any, Any, Any, NULL);
			return TOK_NOP;
		}
		break;
	...
	case TOK_GOTO:		/* goto */
		if (Arg_count <= 1)
			get_string(gotoarg, "Enter a frame number or path: ", nil, 0, FALSE, "goto", "goto");
		else 
			gotoarg(NULL, t);
		return TOK_NOP;
	case TOK_TAB:		/* tab */
...
*/
	...
	case TOK_REREAD:
		...
		if (Arg_count <= 1)
			...
			if (destar = window_arg(Arg_count - 1, Args + 1, 2)) {
				if (destar == savear)
					ar_reread(savear);
				else if (ar_reread(destar) == SUCCESS) {
					/*
					 ...
					 */
					ar_current(destar);
					if (Arg_count != 3 || strcmp(Args[2], "true"))
						ar_current(savear);
				}
			}
		}
	...
	case TOK_UNK_CMD:	/* unknown command, if number, goto, else like open */
		if ((i = atoi(Args[0])) && wdw_to_ar(i) && strspn(Args[0], "0123456789") == strlen(Args[0])) {
			ar_current(wdw_to_ar(i));
			return(TOK_NOP);
		} else {
			if (Args[MAX_ARGS-1])
...
openit:
	...
	case TOK_OBJOP:		/* any other object operation */
		...
		if (objop_args(t) != FAIL)
			objopv(tok_to_cmd(t), Objtype, &Args[1]);
		ar_checkworld(TRUE);
		return TOK_NOP;
	...
	case TOK_UNIX:		/* unix */
		working(TRUE);
		(void) proc_open(0, "UNIX_System", NULL, "sh", "-c", "echo \"To return, type 'exit' or control-d\nYou are in \`pwd\`\"; exec ${SHELL:-/bin/sh}", NULL);
...
int	maxargs;
{
	...
	if (argc <= 0) {
		...
		if ((n = atoi(argv[0])) && (strspn(argv[0], "0123456789") == strlen(argv[0])) && (ret = wdw_to_ar(n)))
			return(ret);
		else {
...
glob_time()
{
	...
	t = time(0L);
	tp = localtime(&t);
	sprintf(buf, "%d:%02.2d:%02.2d %cM", tp->tm_hour % 12 ? tp->tm_hour % 12 : 12, tp->tm_min, tp->tm_sec, tp->tm_hour >= 12 ? 'P' : 'A');
	mess_temp(buf);
}
