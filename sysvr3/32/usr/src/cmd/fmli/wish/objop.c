 ...
 *	All Rights Reserved
 */
#ident	"@(#)vm.wish:src/wish/objop.c	1.5"
	...
	case F_SHELL:
		{
			...
			sprintf(title, "Suspended %s", path_to_title(argvhold[0], NULL));
...
char *path;
{
	...
	if ((ott = path_to_ott(path)) == NULL) {
		...
		mess_temp(nstrcat("Object ", path_to_title(ott_to_path(ott), NULL), 
