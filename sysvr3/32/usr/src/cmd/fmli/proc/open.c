 ...
 *	All Rights Reserved
 */
#ident	"@(#)vm.proc:src/proc/open.c	1.2"
...
#define MAX_PATHSHOW	40	/* max path we will show to wdwmgmt list */
...
char *argv[];
{
	...
	if (Proc_list)
		(void) ar_close(Proc_list);

	PR_all[a.id].ar = ar_create(&a);

	return(ar_current(PR_all[a.id].ar)?SUCCESS:FAIL);
}
