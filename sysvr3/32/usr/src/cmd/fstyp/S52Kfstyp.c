...
char	*argv[];
{
	...
	dev = argv[1];
	if (stat(dev, &buf) < 0) {
		fprintf(stderr, "S52Kfstyp: cannot stat <%s>\n",dev);
		exit(1);
	}

	if (((buf.st_mode & S_IFMT) != S_IFBLK) 
		&& ((buf.st_mode & S_IFMT) != S_IFCHR)) {
		fprintf(stderr,"S52Kfstyp: <%s> is not a block, or a character device\n",dev);
		exit(1);
	}
	...
	if ((fd = open(dev, O_RDONLY)) < 0) {
		fprintf(stderr,"S52Kfstyp: cannot open <%s>\n",dev);
		exit(1);
	}
	
	if (lseek(fd, (long)SUPERBOFF, 0) < 0
		|| read(fd, &sblock, (sizeof sblock)) != (sizeof sblock)) {
		fprintf(stderr,"S52Kfstyp: cannot read superblock\n");
		close(fd);
		exit(1);
	}
	
	/*
	 *	Determine block size and if it is type S52K
 	 */
	
	if (sblock.s_magic == FsMAGIC) {
		switch (sblock.s_type) {
			case Fs1b:
				fprintf(stderr, "S52Kfstyp: <%s> is not type S52K, invalid block size for type S52K\n", dev);
				exit(1);
				break;
			case Fs2b:
				result = s5bsize(fd);
				if (result == Fs2b) {
					...
					fprintf(stderr, "S52Kfstyp: cannot determine block size\n");
					exit(1);
				}
				break;
			case Fs4b:
				...
				break;
			default:
				fprintf(stderr,"S52Kfstyp: cannot determine file system type\n");
				exit(1);
		}
	} else {
		fprintf(stderr,"S52Kfstyp: <%s> is not a System V file system\n",dev);
		exit(1);
	}
	close(fd);

	if (sanity_check(&sblock,bsize) == FAILURE) {
		fprintf(stderr,"S52Kfstyp: System V file system sanity check failed\n");
		exit(1);
	}
	fprintf(stdout,"S52K\n");
	exit(0);
