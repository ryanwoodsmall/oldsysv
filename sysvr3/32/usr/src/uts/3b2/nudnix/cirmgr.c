...
register struct token *tokenp;
{
	...
	DUPRINT1(DB_GDPERR,"getcircuit: circuit doesn't exist\n");
	u.u_error = ECOMM;
	return(NULL);
}
		...
		case T_INFO_ACK:
{
			...
			ulong rq = (uint)sizeof(struct request);
...
LABEL(GDP1);
		/* header is completely assembled */
			...
			if (pullupmsg(gp->hdr, sizeof(struct message)) == 0) {
				...
				if (bp && bp != (mblk_t *)1)
					putbq(qp, bp);
				if (!bufcall(sizeof(struct message), BPRI_MED, qenable, qp))
					timeout(qenable, (char *)qp, HZ);
				return;
			}
...
register struct gdp *gdpp;
{
	...
	{
	    DUPRINT4(DB_GDPERR,"bad fd to init_circuit fd = %x, fp = %x, stp = %x\n", fd, fp, stp );
	    u.u_error = EBADF;
	    return(NULL);
	}
