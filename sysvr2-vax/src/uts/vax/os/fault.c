 ...
 *	psl	-> psl when fault occurred
 */
pfault(typ, vaddr, pt, psl)
register pte_t	*pt;
{
	register pfd_t	*pfd;
	register reg_t	*rp;
...
#ifdef OSDEBUG
	if ((oldspl = mfpr(IPL)) != 0)
		printf("pfault: high spl\n");
#endif
	...
	/*	This is purportedly a fix for a microcode bug.
	 *	It comes to us from the old VAX fault code.
	 */
	
	if (typ != MODIFY) {
		if (!typ && ((vaddr>>30) == 1) && cputype == 750)
			typ = MODIFY;
		else {
			return(0);
		}
	...
	if (!pt->pgm.pg_v) {
		regrele(rp);
		if ((tmp = vfault(typ, vaddr, pt, psl)) <= 0)
			return(tmp);
		reglock(rp);
	}
...
#ifdef OSDEBUG
	if ((oldspl = mfpr(IPL)) != 0)
		printf("vfault: high IPL\n");
#endif

	ASSERT((unsigned)vaddr < 0x7ffff800);

	/*	Check for reference bit fault.  See getpages.c
	 */
	if (pt->pgi.pg_pte & (PG_REF | PG_NDREF)) {
	...
	case DBD_FILE: {
		...
		while ( (ptep < endreg) &&
			...
			if (type == DBD_SWAP) {
				 ...
				 * hack. If we're validating the first
				 * page from a disk block, we might as
				 * well try the next one, too, regard-
				 * less of the prepage factor
				 */
				if ((count == tune.t_prep-1) &&
					(dbdp->dbd_adjunct & DBD_FP))
					maxprepage++;
			}
		...
		if (type == DBD_SWAP) {
			...
			while (count > 0) {
				...
				if (dbdp->dbd_adjunct & DBD_FP)
					bufadr = bp->b_un.b_addr;
				else
					bufadr = bp->b_un.b_addr + NBPP;
