...
/*
	u.u_hdr = sizeof(struct filhd) + sizeof(struct naout) +
		ep->ef.nscns*sizeof(struct scnhdr);
*/
	/*
	 * Setup pointers to virtual address space
	 */
	if ((ep->ef.magic == 0570) || (ep->ef.magic == 0575)) {
		bcopy(&ep->af, &u.u_exdata, sizeof(struct ufhd));
		u.u_exdata.ux_datorg = u.u_exdata.ux_unused;
		u.u_exdata.ux_txtorg = u.u_exdata.ux_entloc;
		 ...
		 * Old pre 5.0 format
		 */
		bcopy(&ep->ef, &u.u_exdata, sizeof(struct ufhd));
