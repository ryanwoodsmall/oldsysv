...
loop:
	...
	for (p = &proc[0]; p < (struct proc *)v.ve_proc; p++) {
		...
		cputime = (uservad->u_utime + uservad->u_stime) / HZ;

		if (cputime > (uservad->u_agetime + tune.t_age)) {
			uservad->u_agetime = cputime;
			p->p_flag |= SLOCK;
			ageprocess(p);
			p->p_flag &= ~SLOCK;
		}
