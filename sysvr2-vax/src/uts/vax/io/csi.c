...
#define CSIDEV(X)  (X&0377)
#define KMCDEV(X)  (((unsigned char)X>>4)&017)
#define KMCARG1		(KMCDEV(cp->mdev)<<8)|(cp->mdev&07)
...
int	(*rint)();	/* Protocol driver "interrupt" routine */
{
	...
	mdev = (((char)dev<<4)|(((unsigned char)dev>>5)&017))&0377;
	if((mdev>>4) >= kmc_cnt || (mdev&017) > 7)
		return(BADDEV);
...
struct csiflags *flagp;	/* The start time flags */
{
	...
	if (((kp = &csik[KMCDEV(cp->mdev)])->state & KMCRUN) == 0) {
		if (kmcset(KMCDEV(cp->mdev), CSIINTR, csirint,OKOPT)) {
			return(STERR);
		}
		if ((kmcload(KMCDEV(cp->mdev)<<8, BASEIN, kmc_dmk[KMCDEV(cp->mdev)], NOARG)) == -1) {
			return(STERR);
		}
		kp->state |= KMCRUN;
	}
	...
	if (kmcload(KMCARG1, RUNCMD, opts, idev) == -1) {
		return(STERR);
	}
...
int	idev;		/* The CSI interface index */
{
	...
	if ((cp->state&C_RUN) == 0)
		return(0);
	sps = spl5();
	if (kmcload(KMCARG1, HALTCMD, NOARG, NOARG) == -1) {
		rp->unsolp = &unsolrpt;
...
csirint(dev, code, sel4, sel6)
{
	...
	case RRTNXBUF:
		...
		while ((cp->xqkmc.l_ql < cp->xmax) && (bdp1 = csideq(&cp->xmtq))) {
			addr = ubmdata(bdp1);
			if (kmcload(KMCARG1, XBUFINCMD, loword(addr), hiword(addr)) == -1) {
				rp->unsolp = &unsolrpt;
				rp->unsolp->excode = CSINONAPP;
				rp->unsolp->code = PCDCMD;
				(*cp->rint)(CSIDEV(dev), cp->pindex, CSITERM, rp);
	...
	case RRTNRBUF:
		...
		while ((cp->eqkmc.l_ql < cp->rmax) && (bdp1 = csideq(&cp->emptq))) {
			addr = ubmdata(bdp1);
			if (kmcload(KMCARG1, RBUFINCMD, loword(addr), hiword(addr)) == -1) {
				rp->unsolp = &unsolrpt;
				rp->unsolp->excode = CSINONAPP;
				rp->unsolp->code = PCDCMD;
				(*cp->rint)(CSIDEV(dev), cp->pindex, CSITERM, rp);
	...
	case OKCHECK:
		...
		for (cp = &csi_csi[0], idev = 0; idev < csi_cnt; ++cp, ++idev) {
			if ((cp->state & C_START) && KMCDEV(cp->mdev) == ((dev>>8)&017)) {
				rp->unsolp = &unsolrpt;
...
char	*cmd;		/* Pointer to the command */
{
	...
	if (kmcload(KMCARG1, SCRIPTCMD, sel4, sel6) == -1) {
		rp->unsolp = &unsolrpt;
		rp->unsolp->excode = CSINONAPP;
		rp->unsolp->code = PCDCMD;
		(*cp->rint)(idev, cp->pindex, CSITERM, rp);
...
int	nbytes;		/* Size to be allocated */
{
	...
	if (csibufp->nexus == 0)
		...
		for (i=0; ((csibufp->nexus>>i)&01) == 0; i++);
		if ( (index = ubaindex(i)) < 0 )
			return(NULL);
	}
	csibufp->nexus = index;
	ubmd = mubmalloc(nbytes,0,index);
	csibufp->bubm = mubmvad((paddr_t)segp,ubmd,index);
	csibufp->segp = segp;
#endif
	csibufp->ubmd = ubmd;
...
#ifdef vax
	if (bdp->d_uba != kmc_uba[KMCDEV(cp->mdev)])
		return(-1);
#endif
	sps = spl5();
	csienq(bdp, &cp->xmtq);
...
unsigned char	type;
{
	...
	trsave(0, idev%16, (char *) & csient, sizeof(csient));
	splx(sps);
}
...
csiclr()
{
	...
	for (i=0; i<csibnum; i++) {
		if((csibp = csibpt[i]) != NULL)
			csibp->bubm = mubmvad((paddr_t)csibp->segp,csibp->ubmd,csibp->nexus);
	}
#endif
	...
	for (dev = 0; dev < csi_cnt; ++dev) {
		...
		if (cp->state&C_ATTACH) {
			...
			if ((kp->state & KMCRUN) && (kp->state & KMCRESET) == 0) {
				...
				if (kmcload(KMCDEV(cp->mdev)<<8, BASEIN, NOARG, NOARG) == -1) {
					rp->unsolp = &unsolrpt;
					rp->unsolp->excode = CSINONAPP;
					rp->unsolp->code = PCDCMD;
					(*cp->rint)(dev, cp->pindex, CSITERM, rp);
			...
			if (cp->state & C_START) {
				cp->state |= C_RESET;
				opts = ((short)cp->flags.options << 8) | ((short)cp->flags.intopts & 0377);
				if (kmcload(KMCARG1, RUNCMD, opts, dev) == -1) {
					rp->unsolp = &unsolrpt;
					rp->unsolp->excode = CSINONAPP;
					rp->unsolp->code = PCDCMD;
					(*cp->rint)(dev, cp->pindex, CSITERM, rp);
