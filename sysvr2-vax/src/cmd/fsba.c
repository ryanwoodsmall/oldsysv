...
*	from 512 bytes/block to 1024 bytes/block for vax and 3B
*	or 512 bytes/block to 2048 bytes/block for 3B5.
*/
#include <stdio.h>
#include <sys/param.h>
#include <sys/types.h>
...
char *argv[];
{
	...
	while (--argc) {
		...
		printf("%s\n", dash);
#ifdef u3b5
		printf(" for new file system (2048bytes/block)\n");
#else
...
char *f;
{
	...
	case Fs2b:   
#ifdef u3b5
	     printf(" %s is a 2k byte block file system already\n", f);
#else
