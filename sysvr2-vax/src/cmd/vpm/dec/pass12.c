...
#define PSIZE	1800			/* PAP size */
#define SCRSIZE 1536			/* max # of instructions permitted */
					/* when atoe and etoa tables used  */
...
#define CHECK_i if(i >= SCRSIZE - 1) error('i',i);
#define CHECK_j if(j >= TABLESZ - 1) error('j',j);

static char Sccsid[] = "@(#)pass12.c	1.5";
...
#ifdef DEBUG
	fprintf(stderr,"pss12: i = %d, j = %d (dec)\n",i,j);
	fprintf(stderr,"       i = %o, j = %o (oct)\n",i,j);
#endif
	if(nv > 0) --nv;   /* don't touch nv if nv==0; this is because SSTACK */
			   /* must be defined to be < 256 if it is to fit into*/
			   /* a KMC register */
	Stack -= nv;
...
/*
 * This function issues mesgs to the user (and exits) when this programs
 *  internal tables overflow, and it warns the user when the compiled
 *  script approaches the maximum size that can be downloaded into a
 *  kmc/kms.
 */
error(ind,indval)
char ind;
int indval;
{
	static int warned = 0;		/* indic. whether user warned script */
					/* is near maximum permitted size    */          
	if (ind == 'j') {
		fprintf(stderr,"pss12: error - symbol table overflow\n");
		exit(3);
	}
	else {
		if(indval >= PSIZE - 1) {
			fprintf(stderr,"pss12: error - script too large, instruction table overflow\n");
			exit(3);
		}
		else {
			if(warned == 0) {
				fprintf(stderr,"pss12: warning - compiled script approaching max. permitted size\n");
				fprintf(stderr,"  - additional error mesgs will be ouput if size limit exceeded\n");
				warned = 1;	/* now user has been warned */
			}
		}
	}
}
