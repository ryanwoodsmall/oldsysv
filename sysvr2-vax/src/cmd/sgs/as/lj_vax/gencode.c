...
static char noshort = 0; /* flag set to prohibit the generation of short
			    literals while generating span dependent instrs.
			    The generation of short literals royally screws
			    up the common sdi logic. */
...
{"jcc",		0x1f}, /* bcc dest / bcs .+6; jmp dest */
{"jcs",		0x1e}, /* bcs dest / bcc .+6; jmp dest */
{"jeql",	0x12}, /* beql dest / bneq .+6; jmp dest */
{"jeqlu",	0x12}, /* beqlu dest / bnequ .+6; jmp dest */
{"jgeq",	0x19}, /* bgeq dest / blss .+6; jmp dest */
{"jgequ",	0x1f}, /* bgequ dest / blssu .+6; jmp dest */
{"jgtr",	0x15}, /* bgtr dest / bleq .+6; jmp dest */
{"jgtru",	0x1b}, /* bgtru dest / blequ .+6; jmp dest */
{"jleq",	0x14}, /* bleq dest / bgtr .+6; jmp dest */
{"jlequ",	0x1a}, /* blequ dest / bgtru .+6; jmp dest */
{"jlss",	0x18}, /* blss dest / bgeq .+6; jmp dest */
{"jlssu",	0x1e}, /* blssu dest / bgequ .+6; jmp dest */
{"jneq",	0x13}, /* bneq dest / beql .+6; jmp dest */
{"jnequ",	0x13}, /* bnequ dest / beqlu .+6; jmp dest */
{"jvc",		0x1d}, /* bvc dest / bvs .+6; jmp dest */
{"jvs",		0x1c}, /* bvs dest / bvc .+6; jmp dest */
{"jbr",		0x17}, /* brb dest / jmp dest */
{"jbc",		0xe0}, /* bbc dest / bbs .+6; jmp dest */
{"jbs",		0xe1}, /* bbs dest / bbc .+6; jmp dest */
{"jbcc",	0xe4}, /* bbcc dest / bbsc .+6; jmp dest */
{"jbsc",	0xe5}, /* bbsc dest / bbcc .+6; jmp dest */
{"jbcs",	0xe2}, /* bbcs dest / bbss .+6; jmp dest */
{"jbss",	0xe3}, /* bbss dest / bbcs .+6; jmp dest */
{"jlbc",	0xe8}, /* blbc dest / blbs .+6; jmp dest */
{"jlbs",	0xe9}  /* blbs dest / blbc .+6; jmp dest */
};
...
addrmode op1,op2,op3;
	...
	case EXTIN1: {  /* short = 2 bytes; long = 8 bytes */
			/* operands = dest */
			if (opt) {
			...
			generate(BITSPBY,GENVALUE,(long)nsdiop[iptr].nopcode,
								NULLSYM);
			generate(BITSPBY,GENVALUE,6L,NULLSYM); /* .+6 operand */
			generate(BITSPBY,GENVALUE,0x17L,NULLSYM); /* jmp inst */
			genopr(op1,1,INSTL); /* dest operand */
			break;
		     };

	case EXTIN2: {  /* short = 2 bytes; long = 6 bytes */
			/* operands = dest */
			if (opt) {
				switch(shortsdi(op1.adexpr.symptr,
	...
	case EXTIN3: {  /* short = 7 bytes; long = 13 bytes */
			/* operands = long, byte, dest */
			if (opt) {
			...
			generate(BITSPBY,GENVALUE,6L,NULLSYM); /* .+6 operand */
			generate(BITSPBY,GENVALUE,0x17L,NULLSYM); /* jmp inst */
			genopr(op3,3,INSTL); /* dest operand */
			break;
		     };
	
	case EXTIN4: {  /* short = 6 bytes; long = 12 bytes */
			/* operands = long, dest */
			if (opt) {
			...
			generate(BITSPBY,GENVALUE,6L,NULLSYM); /* .+6 operand */
			generate(BITSPBY,GENVALUE,0x17L,NULLSYM); /* jmp inst */
			genopr(op2,2,INSTL); /* dest operand */
			break;
		     };
