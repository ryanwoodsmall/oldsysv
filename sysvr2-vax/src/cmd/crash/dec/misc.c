...
init()
{
	...
	if(lseek(mem, (long)SYM_VALUE(V), 0) == -1) perror("lseek");
	if(read(mem, &v, sizeof v) != sizeof v)
...
getuarea(slot)
{
	...
	if(slot == -1) {
		if(lseek(mem,(long)SYM_VALUE(Curproc),0) == -1) {
			error("seek error on curproc");
			return(-1);
		}
	...
	if(lseek(mem,(long)
		SYM_VALUE(Proc)+slot*sizeof(struct proc),0) == -1) {
			error("seek to proc slot");
			return(-1);
	}

	if(read(mem, (char *)&pbuf, sizeof(pbuf)) == -1) {
	...
	if(read(mem,(char *)&sbrpte, sizeof sbrpte) == -1) {
