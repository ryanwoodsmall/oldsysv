...
#ifdef ONEPASS
	printf("L%d:", ftlab1 );
	if( spoff ){ /* adjust stack for autos */
		printf( "	sub	$%ld.,sp\n", spoff );
		}
	printf("	jbr	L%d\n", ftlab2 );
#else
...
zzzcode( p, c, cook ) register NODE *p; {
	register m;
	switch( c ){
	...
	case 'Y':
		/* do a clear of the high byte for TUCHAR */
		if( p->in.type == UCHAR )
			expand( p, FOREFF, "	bic	$!377,A1\n");
		return;
	...
	case 'H':  /* fix up unsigned shifts */
		{	register NODE *q;
			...
			if( p->in.right->in.op != ICON && p->in.left->in.op != REG ) cerror( "SH left bad" );

			r = p->in.left->tn.rval;
			t = p->in.left->in.type;
			l = (t==LONG || t == ULONG );

			if( !ISUNSIGNED(t) ) return; /* signed is ok */
			...
			if( p->in.right->in.op == ICON ){
				...
				if( s < 16 ){ /*mask int, char and upper long */
					if( s == 1 && t == UCHAR ){/* follows an asrb */
						printf( "\tbicb	$0200," );
						}
					else  if( p->in.type != UCHAR ){
						m = 0100000;
						m >>= s;  /* sign extends... */
						m <<= 1;
						printf( "\tbic	$%o,", m );
	...
	case 'R':
		{
			...
			if( p->in.op != STASG ) cerror( "STASG bad" );

			l = p->in.left;
			r = p->in.right;


			count = p->stn.stsize / 2;

				/* The only way to tell if a register is free
				 * is from the table match. We need a way to
				 * push a register variable on the stack and
				 * restore it later.
				*/
			if( (c == 'R') && (count > 4) ){ /* put it in a loop */
				NODE dummy; /* dummy node for expand */
				m = getlab();
				dummy.stn.stsize = count;/* set size for ZW */
				dummy.in.op = STASG;
				expand( &dummy, FOREFF, "\tmov	$ZW,A1\n");
				deflab(m);
				printf("\tmov\t(r%d)+,(r%d)+\n",r->tn.rval,l->tn.rval);
	/*jwf use these to generate code when the optimizer is fixed
				expand( &dummy, FOREFF, "\tsob	A1,");
				printf("L%d\n", m);
	*/
				expand( &dummy, FOREFF, "\tdec	A1\n");
				expand( &dummy, FOREFF, "\tjne	");
				printf("L%d\n", m);
				}
			else {
				while( count-- ){ /* simple load/store */
					printf("\tmov\t(r%d)+,(r%d)+\n",r->tn.rval,l->tn.rval);
					}
				}
...
optim2( p ) register NODE *p; {
	...
	case ASG AND:
		...
		break;
	case UNARY MUL:
		/*JWF rewrite for efficient address computation */
		t = p->in.left;
		if( t->in.op == PLUS &&
				t->in.right->in.op == LS &&
					(r = t->in.left)->in.op == MINUS ){
			p->in.left = r; /* move the MINUS */
			t->in.left = r->in.left; /* move REG node */
			r->in.left = t; /* move the PLUS node */
		}
		break;
...
myreader(p) register NODE *p; {
	...
	walkf( p, optim2 );	/* convert & for pdp bic	*/
	canon( p );		/* expands r-vals for fileds */
/* This call to optim2() has been moved to ffld() in reader.c
	to catch nodes which are converted after myreader().
	It was moved up befor canon to catch the normal & to bic.
	walkf( p, optim2 );
*/
