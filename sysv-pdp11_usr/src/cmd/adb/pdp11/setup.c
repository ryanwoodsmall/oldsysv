...
setsym()
{
	...
	THEN	magic=txthdr[0];
		IF magic!=0411 ANDF magic!=0410 ANDF magic!=0407 ANDF magic!=0405 ANDF magic!=0437
		THEN	magic=0;
		ELSE	symnum=txthdr[4]/SYMTABSIZ;
			IF magic == 0437
			THEN	txtsiz = ((long)(txthdr[6]&0177400) << 8 ) + txthdr[1];
			ELSE	txtsiz = txthdr[1];
			FI
			datsiz=txthdr[2];
			symbas=txtsiz+datsiz;
			txtmap.b1=0;
			txtmap.e1=(magic==0407?symbas:txtsiz);
			txtmap.f1 = TXTHDRSIZ+(magic==0437?datsiz:0);
			txtmap.b2=datbas=(magic==0410?round(txtsiz,TXTRNDSIZ):0);
			txtmap.e2=txtmap.b2+(magic==0407?symbas:datsiz);
			txtmap.f2 = TXTHDRSIZ+((magic==0407||magic==0437)?0:txtmap.e1);
			entrypt=txthdr[5];
			relflg=txthdr[7];
