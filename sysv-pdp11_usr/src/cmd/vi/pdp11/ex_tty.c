...
gettmode()
{
	...
	if (gtty(1, &tty) < 0)
		return;
	if (ospeed != tty.sg_ospeed & 017)
		value(SLOWOPEN) = tty.sg_ospeed < B1200;
	ospeed = tty.sg_ospeed & 017;
	normf = tty.sg_flags;
...
char **sstrs[] = {
	...
	&ED, &EI, &HO, &IC, &IM, &IP, &LL, &MA, &ND, &xNL,
	&xPC, &RC, &SC, &SE, &SF, &SO, &SR, &TA, &TE, &TI, &UP, &VB, &VS, &VE,
	&DOWN_PARM, &UP_PARM, &AL_PARM, &DL_PARM
};
bool *sflags[] = {
	...
	char *type;
{
	...
	if (tgoto(DOWN_PARM, 2, 2)[0] == 'O')	/* OOPS */
		DOWN_PARM = 0, costDP = 10000;
	else
		costDP = strlen(tgoto(DOWN_PARM, 8, 10));
	PC = xPC ? xPC[0] : 0;
	aoftspace = tspace;
