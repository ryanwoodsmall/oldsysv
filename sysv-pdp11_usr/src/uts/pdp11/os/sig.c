...
core()
{
	...
	if (!access(ip, IWRITE) && (ip->i_mode&IFMT) == IFREG) {
		...
		writei(ip);
		u.u_base = u.u_sep ? 0 : (((u.u_tsize+127)>>7)<<13);
		u.u_count = ctob(u.u_dsize);
		u.u_segflg = 0;
		writei(ip);
		for (s = 0, bitm = u.u_mbitm; bitm; s++, bitm >>= 1) {
			if (bitm&01) {
				u.u_base = s << 13;
				u.u_offset = ctob((s<<7) + USIZE);
				u.u_count = ctob(((u.u_uisd[8+s]&077400)>>8)+1);
				writei(ip);
			}
		}
		u.u_base = ctob(u.u_dsize);
		u.u_count = ctob(u.u_ssize);
		u.u_mbitm = 0;
		estabur((unsigned)0, u.u_dsize + u.u_ssize, (unsigned)0, 0, RO);
		writei(ip);
	} else
		u.u_error = EACCES;
	iput(ip);
