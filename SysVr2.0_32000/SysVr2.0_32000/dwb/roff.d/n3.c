...
int name;
{
	...
	hname = name & NAMEMASK;
	htoff = toff = HASH(name,NM);	/* save orig value */
...
rbf(){
	...
	if (!(i = rbf0(&ip)))	{
#else INCORE
	if ((ip.b == (filep)-1) || (ip.b == (filep)-2))
		i = -1;
	   else i = (int) memblist[ip.b][ip.o];
	if (!i)	{
#endif INCORE
		i = popi();
...
collect()
{
	...
	if (((argpp = (int **)malloc(MARGL*sizeof(int))) == NULL) ||
	    ((savnxf = (struct s *) malloc(sizeof(struct s))) == NULL))	{
		prstr("out of memory for macro call");
		exit(1);	}
	hsstrp = (int *)argpp;
	...
	sstrp = hsstrp;
	free(nxf);
	nxf = savnxf;		/* caution: string may have finished expanding
