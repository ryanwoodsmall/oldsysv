...
#ifdef DEBUG
short dblevel = 0;		/* debug flags */
short dbexpand = 0;		/* expand getch's */
char dbname1[NMACS], dbname2[NMACS];	/* names of called macros */

#include <stdio.h>

#define MACALLS 1		/* show macro calls */
#define REQCALLS 2		/* show request calls */
...
char **argv, **envp;
{
	...
	while(--argc > 0 && (++argv)[0][0]=='-')
		...
		case 'c':	/* read compacted macros */
			...
			read(i, eblock, sizeof(eblock));	/* get environment blocks*/

			read(i, blist, sizeof(blist));		/* macro storage list */

			for (j=0, k=0;  j<NBLIST;  j++)
			    if (blist[j]) k++;

			free(malloc(BLK * sizeof(tint) * (k + 5)));	/* pre-alloc total space */

			for (j=0;  j<NBLIST;  j++)
			    if (blist[j])	{
				memblist[j] = (tint *) malloc(BLK * sizeof(tint));
				read(i, memblist[j], BLK * sizeof(tint));	}
#endif INCORE
		...
		case 'T':
			...
			q = &argv[0][2];
			if(!((*q) & 0177))continue;
			if (*q == '/') p = termtab;	/* explicit pathname */
			while((*p++ = *q++) != 0);
#ifdef tso
			*p++ = '.';	/* fudge name on tso */
			*p++ = 't';
		...
		case 'g':
			...
			continue;
		case 'T':
			if (strcmp(&argv[0][2], "cat") == 0)
				continue;
			if (argv[0][2] == '/') ffi = 0;	/* explicit path */
				else ffi -= 2;		/* overwrite ft */
...
int a,b;
{
	...
	if (nametab[j].ename & MMASK)	{
		...
		if(b)collect();
		flushi();
		retval = pushi((filep)nametab[j].vv.val);
#ifdef DEBUG
		if (dblevel & MACALLS)	{
		    flusho();			/* flush partial buffers */
		    fprintf(stderr, "*** MACRO %c%c%c", (nb ? c2 : cc),
			    dbname1[maclev] = nametab[j].ename & 0177,
			    dbname2[maclev] = (nametab[j].ename>>BYTE) & 0177);
		    for (i=1; i<=frame->nargs; i++)	{
			fprintf(stderr, " \"");
#ifndef INCORE
			for (k=(*((int **)frame+i-1+(sizeof(struct s)/sizeof(int **)))); *k; k++)
#else
			for (k=(int *)*(argsp+i-1); *k; k++)
#endif
			    fprintf(stderr, "%c", (char) *k);
			    fprintf(stderr, "\"");	}
		    fprintf(stderr, "\n");	}
#endif DEBUG
		return retval;	}
		...
		if (dblevel & REQCALLS)	{
		    flusho();
		    fprintf(stderr, "*** REQUEST %c%c%c",
			(nb ? c2 : cc),
			nametab[j].ename & 0177,
			(nametab[j].ename>>BYTE) & 0177);
		    dbexpand++;		}
#endif
		retval = (*nametab[j].vv.f)(0);
#ifdef DEBUG
		dbexpand = 0;
#endif
		return retval;	}
...
#ifdef DEBUG
	if (dblevel & MACALLS)
	    fprintf(stderr, "*** RETURN .%c%c\n", dbname1[maclev], dbname2[maclev]);
#endif
}
...
#ifdef DEBUG
	if (dbexpand) fprintf(stderr, "%c", i);
#endif
	return(i);
}
...
popf(){
	...
	ioff = offl[--ifi];
	cd = cfline[ifi];		/* restore line counter */
...
caseco()		/* perform .co request */
{
	...
	write(i, eblock, sizeof(eblock));	/* environments */
	write(i, blist, sizeof(blist));		/* used block list */

	for (j=0;  j<NBLIST;  j++)
	    if (blist[j])
		write(i, memblist[j], BLK * sizeof(tint));	/* write buffers */
#endif INCORE
...
caseunix()	/* read output of command sent to unix */
{
	...
	register int i, j;
	register char *p;
	char unixcmd[300];	/* hold the UNIX command */

	lgf++;
	copyf++;
	skip();			/* prepare to read unix command */

	for (p=unixcmd, i=0;  i < sizeof(unixcmd)-1;  i++)	{
