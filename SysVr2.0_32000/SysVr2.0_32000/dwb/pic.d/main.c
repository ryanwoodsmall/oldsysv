...
struct	obj	**objlist = 0;	/* store the elements here */
...
/* You may want to change this if you don't have an APS... */
	...
	char **argv;
{
	...
	while (argc > 1 && *argv[1] == '-') {
		...
		case 'T':
			if (strcmp(&argv[1][2], "aps") == 0) {
				...
				|| strcmp(&argv[1][2], "i10") == 0) {
				res = 240;
				devtype = DEV450;
			} else if (strncmp(&argv[1][2], "X97", 3) == 0) {
				res = 300;
				devtype = DEVX97;
				DX = DY = 1 ;
			} else {
	...
	setdefaults();
	objlist = (struct obj **) grow(objlist, "objlist",
		nobjlist += 100, sizeof(struct obj *));
	if (argc <= 1) {
		yyin = stdin;
		getdata(yyin);
	} else
	...
	int num, size;
{
	...
	if (p == NULL) {
		yyerror("can't grow %s to %d", name, num * size);
		exit(1);
	}
	return p;
}
