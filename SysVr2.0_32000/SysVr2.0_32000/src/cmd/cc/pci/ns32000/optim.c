...
********************************************************************************
*/
/*	static char  ID[] = "@(#) optim.c: 2.3 10/7/82";	*/

/*	machine independent improvement routines */

#include "optim.h"

#define BID(b) ((b) ? (b)->index : 0)
...
mrgbrs() { /* merge branches to unconditional branches */
	...
	for (ALLB(b, b0.next))
		...
		else if (bb != NULL && bb != b && (b->firstn->ops[0][0] != '_') &&
...
outdeg(b) register BLOCK *b; { /* compute indeg of loop exit target */
	...
	register int indeg = br->indeg;
	BLOCK *looptop;

	if (indeg <= 1)
		return (indeg); /* all done if indeg <= 1 */
	for (ALLB(bb, b0.next))
		bb->marked &= ~B_EXIT;
	for (bb = b->nextl; bb != b && !(bb->marked & B_EXIT); ) {
		bb->marked |= B_EXIT;
		if (bb->nextr == br && --indeg <= 1) /* this is a loop exit */
			break; /* all done if indeg <= 1 */
		if ((bb = bb->nextl) == NULL)
			break;
		/* exit from inner loops */
		looptop = bb;		/* remember top to prevent hung optim */
		while ((bb->marked & B_EXIT) &&
		    (bb = bb->nextr) != NULL &&
		     bb != looptop)
			if (bb->nextr == br && --indeg <= 1) {
				bb = NULL; /* all done if indeg <= 1 */
				break;
			}
		if (bb == NULL)
			break;
   ...
   char *s;
{
   ...
   for (ALLB(b, b0.next))
   {
           register NODE *p = b->firstn->back;

           PRINTF("*** block -%d(%d,%d,%d)- ***\n", BID(b), BID(b->next), BID(b->nextl), BID(b->nextr));
           do {
                   p = p->forw;
                   prinst(p);
           } while (p != b->lastn);
