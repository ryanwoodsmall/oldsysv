...
/*printf( "# op=0x%x\tsubop=0x%x\tforw=0x%x\tback=0x%x\tcode=%s\n",
	p->op, (unsigned char) p->subop, p->forw, p->back, p->code );/**/
	switch (p->op) {
	...
	{
		/*
		** Change multiplication by constant powers of 2 to shifts.
		*/
		splitrand(p);
...
ashadd:
	...
	case EXTZV:
	{
		...
		**
		** also hidden field comparisons in either direction:
		**	extv	A,B,C,rD	\	cmpv	A,B,C,E
		**	cmpl	rD,E		 >	[branch]
		**	[branch]		/
		**
		**	extv	A,B,C,rD	\	cmpv	A,B,C,E
		**	cmpl	E,rD		 >	[reversed branch]
		**	[branch]		/
		*/
		register struct	node	*pf;	/* forward node */
		register struct	node	*pn;	/* next node (after pf) */
		register int		extreg;	/* register extracted into */
		register int		flen;	/* field length */

		pf = p->forw; pn = pf->forw;
		splitrand(p);
		extreg = isreg(regs[RT4]);
		if (regs[RT2][0] != '$') goto try_1;
		if ((flen = getnum(&regs[RT2][1])) < 0) goto try_1;
		switch (flen)
		{
		case 1:		/* single-bit field */
		{
			if (extreg < 0 || extreg >= NUSE) goto try_1;
			if (pf->op != TST) goto try_1;
			if (uses[extreg] && uses[extreg] != pf) goto try_1;
			if (pn->op != CBR) goto try_1;
			if (pn->subop != JEQ && pn->subop != JNE) goto try_1;
			splitrand(pf);
			if (extreg != isreg(regs[RT1])) goto try_1;
		...
		case 8:		/* byte-size field */
		case 16:	/* word-size field */
		{
			register int	boff;	/* bit offset */
			register int	coff;	/* chunk (byte or word) offset*/

			if (regs[RT1][0] != '$') goto try_1;
			if ((boff = getnum(&regs[RT1][1])) < 0) goto try_1;
			coff = boff / flen;
			if (coff && (isreg(regs[RT3]) >= 0)) goto try_1;
			if (boff < 0 || (boff % flen) != 0) goto try_1;
			if (pf->op == TST && uses[extreg] == pf)
			{
				p->op = TST; p->subop = flen == 8 ? BYTE : WORD;
				regs[RT2][0] = '\0'; delnode(pf);
			}
			else
			{
			...
			if (coff == 0)
				...
				sprintf(regs[RT1], "%d%s%s", boff/8,
					regs[RT3][0] != '(' ?"+":"", regs[RT3]);
			if (p->op != TST)
				strcpy(regs[RT2], regs[RT4]);
			regs[RT3][0] = '\0'; regs[RT4][0] = '\0';
			p->pop = 0; newcode(p); nchange++; goto std;
		}
		}
try_1:
		if (pf->op == TST && pf->subop == LONG && pn->op == CBR)
		{
			if (extreg < 0 || extreg >= NUSE || extreg != isreg(pf->code))
				goto try_2;
			p->op = (p->op == EXTV) ? CMPV : CMPZV; p->subop = 0;
			strcpy(regs[RT4], "$0"); uses[extreg] = NULL;
			p->pop = NULL; newcode(p);
			delnode(pf);
			nfield++; nchange++; goto std;
		}
try_2:
		if (pf->op == CMP && pf->subop == LONG && pn->op == CBR)
		{
			int	r1, r2;
			char	*cp, *cp1, *cp2;

			if (extreg < 0 || extreg >= NUSE || uses[extreg] != pf)
				goto std;
			splitrand(pf);
			r1 = isreg(cp1 = copy(regs[RT1]));
			r2 = isreg(cp2 = copy(regs[RT2]));
			splitrand(p);
			cp = NULL;
			if (extreg == r1)		/* 1st case above */
				cp = cp2;
			else
			if (extreg == r2)		/* 2nd case above */
			{
				cp = cp1;
				pn->subop = invbr[pn->subop]; pn->pop = 0;
			}
			if (cp)
			{
				if (r1 >= 0 && r1 < NUSE && uses[r1] == pf)
					uses[r1] = p;
				if (r2 >= 0 && r2 < NUSE && uses[r2] == pf)
					uses[r2] = p;
				delnode(pf);
				strcpy(regs[RT4], cp);
				p->op = (p->op == EXTV) ? CMPV : CMPZV;
				p->pop = 0; p->subop = 0; newcode(p);
				uses[extreg] = NULL;
				nfield++; nchange++; goto std;
			}
		}
...
#if 0		/* these optimizations are not safe in all cases! */
	case MOV:
		if ( ( p->subop & 0xf ) != LONG )
			goto std;
		goto skip_movz;
	case MOVZ:
		if ( ( ( p->subop >> 4 ) & 0xf ) != LONG )
			goto std;
	skip_movz:
		/* fall through... */
	case CVT:
	{
		...
		**
		**			if A not auto inc,dec
		**			note: only produced when
		**				sizeof(Y) < sizeof(long)
		**	movl	A,r0
		**	cvtYl	r0,r0	>	movY	A,B
		**	cvtlY	r0,B
		**	(not ret,cmp!)
		**
		**			if A not auto inc,dec
		...
		**				sizeof(Y) < sizeof(long)
		**	movl	A,r0
		**	movzYl	r0,r0	>	movY	A,B
		**	cvtlY	r0,B
		**	(not ret,cmp!)
		**
		**			if sizeof(Y) < sizeof(X)
		**				and A not auto inc,dec
		**			or sizeof(Y) = sizeof(X)
		**	movzXl	A,r0
		**	movzYl	r0,r0	>	movY	A,B
		**	cvtlY	r0,B
		**	(not ret,cmp!)
		...
		**				or A auto inc,dec
		**	cvtXl	A,r0
		**	cvtlY	r0,B	>	cvtXY	A,B
		**	(not ret,cmp!)
		**
		**			if sizeof(X) < sizeof(Y)
		**				or A auto inc,dec
		**	movzXl	A,r0
		**	cvtlY	r0,B	>	movzXY	A,B
		**	(not ret,cmp!)
		**
		**			if sizeof(X) > sizeof(Y)
		**				and A not auto inc,dec
		**			or sizeof(X) = sizeof(Y)
		**	cvtXl	A,r0
		**	cvtlY	r0,B	>	movY	A,B
		**	(not ret,cmp!)
		**
		...
		**			or sizeof(X) = sizeof(Y)
		**	cvtXl	A,r0
		**	cvtYl	r0,r0	>	cvtYl	A,r0
		**	(not cvt)
		**
		*/
		register struct node	*np = p->forw;
		char			buf[50];
		int			r0r0 = 0;	/* 3 inst. -> 1
							** 2 with r0,r0 -> 2
							*/

		if ( !np->code || ( p->op == CVT && np->op != CVT ) ||
			( p->op != CVT && np->op != MOVZ && np->op != CVT ) ||
			np->forw->op == CMP ||
			np->forw->subop == RET )
		{
			goto std;
		...
		splitrand( np );
		if ( !strcmp( regs[RT1], regs[RT2] ) )
		{
			register struct node *nextp = np->forw;
			...
			{
				r0r0 = 1;
				/* The U(...) is just np->subop reversed */
				if ( nextp->forw->op == CMP ||
					nextp->forw->subop == RET ||
					( U( ( ( np->subop >> 4 ) & 0xf ),
					( np->subop & 0xf ) ) !=
					( nextp->subop ) ) )
				{
		...
		if ( r >= NUSE )
			goto std;
		r = sprintf( buf, "%s,", regs[RT2] );	/* remember length */
		if ( !strcmp( regs[RT1], regs[RT2] ) )
			goto std;
		if ( strncmp( buf, np->code, r ) )
			goto std;
		if ( r0r0 == 1 )
		{
			if ( strncmp( buf, np->forw->code, r ) )
				goto std;
			delnode( np );
			np = p->forw;
			/* Look for auto inc,dec */
			for ( cp1 = regs[RT1]; *cp1; )
				if (*cp1++ == ')' && (*cp1 == '+'||*cp1 == '-'))
					break;
			if ( ( ( p->subop & 0xf ) >
				( r = ( ( np->subop >> 4 ) & 0xf ) ) && !*cp1 )
				|| ( ( p->subop & 0xf ) == r ) )
			{
				r = T( MOV, r );
			}
			else
				r = T( ( p->op == MOVZ ? MOVZ : CVT ),
					U( ( p->subop & 0xf ), r ) );
		}
		else if ( r0r0 == 2 )
		{
			if ( ( p->subop == U(WORD,LONG) &&
				( np->subop == U(BYTE,LONG) ||
				np->subop == U(WORD,LONG) ) ) ||
				( p->subop == U(BYTE,LONG) &&
				np->subop == U(BYTE,LONG) ) )
			{
				r = np->combop;
		...
		np->combop = r;
		(void) sprintf( buf, "%s,", regs[RT1] );
		splitrand( np );
		np->code = copy( buf, regs[RT2] );
		delnode( p );
		np->pop = 0;	/* force lookup in table */
		nchange++;
		goto std;
	}
#endif	/*0*/
	...
	case CMP:
	{
		...
		*/
		...
		splitrand(p);
		if (strcmp(regs[RT1], "$0") == 0)
		{
			/*
			** F77 generates "cmpX $0,r0"; turn it into a
			**	"tstX r0" and reverse the sense of the
			**	following branch.
			**	All branches need to keep
			**	the same equalness sense.
			*/
			switch ( p->forw->subop )
			{
			case JEQ:
			...
			p->op = TST; p->pop = 0;
			strcpy(regs[RT1], regs[RT2]); regs[RT2][0] = '\0';
			newcode(p);
			p->forw->pop = 0; goto std;
		}
...
/* .rx,.rx,.rx */
	...
	case PROBEW:
	case CASE:
/* MOVC3 used to be here, but it's too dangerous:  neither the
** source nor destination operand can be a register (illegal addressing).
*/
/* .rx,.rx */
	case MTPR:
	case CALLS:
	...
	case BIT:
		...
		if (p->op==CASE || p->op==PROBER || p->op==PROBEW)
			lastrand=regs[RT4];
		else
		if (p->op == CMPV || p->op == CMPZV)
			lastrand=regs[RT4+1];
		else
			lastrand=regs[RT3];
		repladdr(p);
...
/* return pointer to register which is "beyond last read/modify operand" */
	...
	switch (p->op) {
		...
		case CMPV: case CMPZV:							return(regs[RT4+1]);
	}
	return(lastrand);
}
		...
		if ((p->subop&0xF)==LONG && *(regs[RT2])!='$') {/* result of EXTZV is long */
			...
			if (p->back->op==CVT || p->back->op==MOVZ) {/* greedy, aren't we? */
				...
				if (equstr(src,cp2) && okio(cp1) && !indexa(cp1)
				  && ((p->back->subop)&0xf) < FLOAT
				  && ((p->back->subop)>>4) < FLOAT
			...
			if ((pos + siz) > 32)	/* falls over the edge */
				return(p);
