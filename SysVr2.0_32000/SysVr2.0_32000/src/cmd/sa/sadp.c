...
/*      sadp.c - For VAX and PDP11 machines,
		...
		once every second.
		For NS32000 system, it profiles the DC.
...
static char Sccsid[]="@(#)sadp.c	1.11";
main(argc,argv)
int argc;
char **argv;
{
...
/*		Its a floppy - only one of these  */
	...
	if (dflg && dn >= ub) { /*  THIS IS NOT CHECKED ANYWHERE!  */
...
/*      take a snapshot of buffer header pool , swap buffer pool and
	physical buffer header  */
		...
		lseek(f,(long)setup[DCU].n_value,0);
		if( read(f,dkp,(sizeof (struct iobuf)* 1)) == -1){
			perror("sadp");
			extmsg("cannot read disk drive iobuf");
		}
