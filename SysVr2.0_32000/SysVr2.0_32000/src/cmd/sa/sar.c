...
/*      process options with arguments and pack options 
	without arguments  */
	while ((i= getopt(argc,argv,"Auybdvcwaqmpro:s:e:i:f:")) != EOF)
		switch(ccc = i){
		case 'o':
			oflg++;
		...
		case 'f':
			...
			break;
		case '?':
			fprintf(stderr,"usage: sar [-ubdycwaqvmpr][-o file] t [n]\n");
			fprintf(stderr,"       sar [-ubdycwaqvmpr][-s hh:mm][-e hh:mm][-i ss][-f file]\n");
			exit(2);
			break;
		default:
