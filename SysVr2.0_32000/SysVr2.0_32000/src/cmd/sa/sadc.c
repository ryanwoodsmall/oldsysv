...
#ifdef u3b2
int idcnt;
struct iotime idstat[2];	/* The number of id drives is #defined in the driver */
...
static char Sccsid[]="@(#)sadc.c	1.20";
main(argc, argv)
char **argv;
int argc;
{
...
/*	get number of hard disk drives for 3B2	*/
	if (setup[IDCNT].n_value !=0){
		lseek(f,(long)setup[IDCNT].n_value, 0);
		if (read(f,&idcnt,sizeof idcnt) == -1)
			perrexit();
	}
#endif /* u3b2 */
	...
	if (setup[APSTATE].n_value !=0){
		...
		for (j=0;j<SINFO;j++){
			if (setup[j].n_value != 0){
				for (k=0;k<tblmap[j];k++){
					if (j == ID) {
						d.devio[i][0] = idstat[j].io_cnt;
						d.devio[i][1] = idstat[j].io_bcnt;
						d.devio[i][2] = idstat[j].io_act;
						d.devio[i][3] = idstat[j].io_resp;
					} else if (j == IF) {
...
tapetbl()
{
	if (setup[TCINFO].n_value) {
		lseek(f,setup[TCINFO].n_value,0);
		read(f,&tcstat,sizeof(tcstat));
	}
}
dsktbl()
{
	if (setup[DCINFO].n_value) {
		lseek(f,setup[DCINFO].n_value,0);
		read(f,&dcstat,sizeof(dcstat));
	}
}
devmv(s,j)
struct iotime *s;
{
	d.devio[j][0] = s->io_cnt;
	d.devio[j][1] = s->io_bcnt;
	d.devio[j][2] = s->io_act;
	d.devio[j][3] = s->io_resp;
}
#endif
#ifdef u3b
tapetbl(x)
