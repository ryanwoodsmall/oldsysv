...
#ifdef ns32000			/* SYS16 physio requires page alignment */
#define SBRK(x)	(char *) (((unsigned) sbrk(x + 512) + 511) & ~0x1ff)
#else
...
stats()
{
	...
	fprintf(stderr,"%u+%u blocks in\n", nifr, nipr);
	fprintf(stderr,"%u+%u blocks out\n", nofr, nopr);
	if(ntrunc)
		fprintf(stderr,"%u truncated blocks\n", ntrunc);
}
