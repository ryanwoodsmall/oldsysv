...
#define SIZEUSER	8
				/* For -q (quick) who */
#define	MAXUSERS	50
char uname[MAXUSERS][9];	/* Holds output array of names.  */
int iarray[MAXUSERS];		/* Pointer to user-ids in ucount. */
		...
		if(allflag || ((utmp->ut_type >= 0 && utmp->ut_type <= FLAGS) &&
		   entries[utmp->ut_type] == TRUE) ) {
			...
			if(ptr != NULL && strncmp(utmp->ut_line,ptr,sizeof(utmp->ut_line)) != 0)
				continue;
			if (qflag) {
				sprintf(uname[ucount++],"%.8s ",&utmp->ut_user[0]);
				continue;
			}
		...
		/* if hflag (header) flag is set print header */
			...
			if (hflag && utmp->ut_type > INIT_PROCESS
					...
					if (nspace)
						printf("\n");
					if(shortflag < 0)
						fprintf(stdout,"NAME       LINE\
         TIME          IDLE    PID  COMMENTS\n");
					else
						fprintf(stdout,"NAME       LINE\
         TIME\n");
					hflag = FALSE;
...
doquick()
{
	...
	for(i=1; i < ucount; i++) iarray[i] = i;
	do {
		swapflag = 0;
		for(i=1; i < ucount; i++) {
			if (strcmp(uname[iarray[i]],uname[iarray[i+1]]) > 0) {
				swapflag++;
				n = iarray[i];
				iarray[i] = iarray[i+1];
				iarray[i+1] = n;
