...
char *file, **wvec;
{
	...
	if (file[0] == '\0') {
		fclose(fp);
		logent("anlwrk","attempt made to use old workfile was thwarted");
		fp = NULL;
		return(0);
	}
	if (fgets(str, MAXRQST, fp) == NULL) {
	...
	/* sanity check for C. file */
	if (str[0] != 'R' && str[0] != 'S' && str[0] != 'X') {
		/* bad C. file - ignore it */
		fclose(fp);
		unlink(file);
		unlink(afile);
		USRF(USR_COMP);
...
register char *dir, *pre;
{
	...
	uid = geteuid();
	gid = getegid();
	while (gnamef(pdir, filename) && (Nfiles < LLEN)) {
...
/*
		entflst(filename);
*/
		strncpy(Filent[Nfiles++], filename, NAMESIZE);
	}
	fclose(pdir);
	return;
}
