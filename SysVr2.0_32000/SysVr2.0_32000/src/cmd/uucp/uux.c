...
ASSERT(fprintf(f, "S %s %s %s -%s %s 0666 %s %s\n", a, b, c, statop?"o":"", d, Nuser, Sfile) >= 0, "CAN'T WRITE", "", 0);\
}
#define GENRCV(f, a, b, c) {\
ASSERT(fprintf(f, "R %s %s %s -%s %s\n", a, b, c, 0?"o":"", Sfile) >= 0, \
"CAN'T WRITE", "", 0);\
}

#ifdef FOWARD
#define GENXUUCP(f, a, b, c) {\
ASSERT(fprintf(f, "X %s %s %s - \n", a, b, c) >= 0, "CAN'T WRITE", "",0);\
}
...
char	**envp;
{
	...
	char xfwdroute[BUFSIZ];  /* route from local sys to xeq system */
	char revroute[BUFSIZ];   /* route from xeq sys to local system */
	char fwdroute[BUFSIZ];   /* route to file from xeq system */
	char rfwdroute[BUFSIZ];  /* route from file to xeq system */
	char tfile1[BUFSIZ];     /* temp var to hold route */
	char droute[BUFSIZ];     /* dest route including file name */
	char *i;
	char *strrchar();
	char *strcat();
#endif
	...
	*cmdp = '\0';
	...
	ret = stat(rxfile,statbuf);
	ASSERT(ret != 0, "RXFILE EXISTS",rxfile,0);
	fprx = ufopen(rxfile, "w");
	ASSERT(fprx != NULL, "CAN'T OPEN", rxfile, 0);
	setbuf(fprx, NULL);
	updjb(); sprintf(tcfile, "%c.%.6s%c%.1s%.4d", DATAPRE, local, 'T', subjob, jobid);
	ret = stat(tcfile,statbuf);
	ASSERT(ret != 0, "TCFILE EXISTS",tcfile,0);
	fpc = ufopen(tcfile, "w");
	ASSERT(fpc != NULL, "CAN'T OPEN", tcfile, 0);
	setbuf(fpc, NULL);
#ifndef FOWARD
	ASSERT(fprintf(fprx, "%c %s %s\n", X_USER, User, local) >= 0, 
		"CAN'T WRITE", rxfile, 0);
#endif
	if (nonoti)
		ASSERT(fprintf(fprx,"%c\n", X_NONOTI) >= 0, "CAN'T WRITE",
			rxfile, 0);
	if (statop)
		ASSERT(fprintf(fprx,"%c %s\n", X_MAILF, Sfile) >= 0,
			"CAN'T WRITE", rxfile, 0);
	...
	while ((ap = getprm(ap, prm)) != NULL) {
		...
		DEBUG(4,"revroute - %s\n",revroute);
		ASSERT(fprintf(fprx, "%c %s %s\n", X_USER, User, (revroute[0]=='\0')?local:revroute) >= 0, "CAN'T WRITE", rxfile, 0);
#endif
		break;
	}
	if (xsys[0] == '\0')
	...
	if (pipein) {
		...
		while (!feof(stdin)) {
			...
			ASSERT(fwrite(buf, 1, ret, fpd) == ret, "CAN'T WRITE",
				dfile, 0);
		}
	...
	while ((ap = getprm(ap, prm)) != NULL) {
...
#endif
			ASSERT(fprintf(fprx, "%c %s %s\n", X_STDOUT, rest,
			 syspart) >= 0, "CAN'T WRITE", rxfile, 0);
			redir = '\0';
			continue;
		}
		...
		if (strncmp(xsys, local, SYSNSIZE) == SAME
		 && strcmp(xsys, syspart) == SAME) {
			if (ckexpf(rest))
				cleanup(2);
			if (redir == '<') {
				ASSERT(fprintf(fprx, "%c %s\n", X_STDIN, rest) >= 0, "CAN'T WRITE", rxfile, 0);
			}
			else
				APPCMD(rest);
			redir = '\0';
		...
		if (strncmp(syspart, local, SYSNSIZE) == SAME) {
			...
			if (redir == '<') {
				ASSERT(fprintf(fprx, "%c %s\n", X_STDIN, dfile)
					>= 0, "CAN'T WRITE", rxfile, 0);
				ASSERT(fprintf(fprx, "%c %s\n", X_RQDFILE, dfile) >= 0, "CAN'T WRITE", rxfile, 0);
			} else {
		...
		if (strcmp(syspart, xsys) != SAME) {
			...
			cflag++;
			if (redir == '<') {
				ASSERT(fprintf(fprx, "%c %s\n", X_RQDFILE, t2file) >= 0, "CAN'T WRITE", rxfile, 0);
				ASSERT(fprintf(fprx, "%c %s\n", X_STDIN, t2file)
					>= 0, "CAN'T WRITE", rxfile, 0);
			} else {
				ASSERT(fprintf(fprx, "%c %s %s\n", X_RQDFILE,
				t2file, (lxp=strrchr(rest, '/'))?lxp+1:rest)
				>= 0, "CAN'T WRITE", rxfile, 0);
				APPCMD((lxp=strrchr(rest, '/'))?lxp+1:rest);
			}
			redir = '\0';
	...
	if (strncmp(xsys, local, SYSNSIZE) == SAME) {
		...
		ASSERT(link(rxfile, tfile) >= 0, "CAN,T LINK", tfile, 0);
		unlink(rxfile);
	...
	if (cflag) {
		...
		ASSERT(link(tcfile, cfile) >= 0, "CAN'T LINK", cfile, 0);
		unlink(tcfile);
