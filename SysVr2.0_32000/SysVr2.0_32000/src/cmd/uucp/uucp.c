...
char	**envp;
{
	...
	if ((cp = strchr(argv[argc - 1], '!')) != NULL) {
		...
		*cp = '\0';
		if (*sysfile2 == '\0' || strncmp( sysfile2, Myname, SYSNSIZE ) == SAME)
			sysfile2 = Myname;
		else  {
	...
	while (argc > 2) {
		if ((cp = strchr(argv[1], '!')) != NULL) {
			...
			*cp = '\0';
			if (strlen(sysfile1) > SYSNSIZE)
				*(sysfile1 + SYSNSIZE) = '\0';
			if (*sysfile1 == '\0' || strncmp(sysfile1, Myname, SYSNSIZE) == SAME)
				sysfile1 = Myname;
			else {
		...
		DEBUG(4, "file1 - %s\n", file1);
		if  (copy(sysfile1, file1, sysfile2, file2))
			errors++;
#ifdef FOWARD
	...
	if (*Ropt != '-')
		if (limit < CDLIMIT)  {
			sprintf(msg,"ULIMIT (%ld) < CDLIMIT (%ld)",limit,
					CDLIMIT);
			logent(User,msg);
		}
		else
...
register int code;
{
	...
	rmlock(CNULL);
	if (code < 0)
		fprintf(stderr, "uucp failed completely: code %d\n", code);
	if (code > 0) {
		fprintf(stderr, "uucp failed partially: %d error", code);
		if (code==1) fprintf(stderr," \n");
