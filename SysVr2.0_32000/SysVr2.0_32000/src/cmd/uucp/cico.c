...
int Stattype[] = {0, 0, 0, 0,
	SS_NODEVICE, SS_DFAIL, SS_FAIL, SS_BADSEQ, SS_BAD_LOG_MCH
	};
...
char **envp;
{
	...
	minlimit = ulimit(1,dummy);
	sprintf(umsg,"ULIMIT (%ld) < CDLIMIT (%ld)",minlimit,CDLIMIT);
	...
	if (Role == SLAVE) {
		...
		DEBUG(4,"return from callcheck: %d",ret);
		if (ret==1) {
			...
			logent("BAD LOGIN/MACHINE NAME", "FAILED");
			systat(Rmtname, SS_BAD_LOG_MCH, "FAILED");
			omsg('R', "LOGIN", Ofn);
			cleanup(0);
	       	     }
		seq = 0;
...
loop:
	strcpy(User, Uucp);
/*
 *  Ensure ulimit is at least as great as the default ulimit (CDLIMIT)
 */
	ASSERT(minlimit >= CDLIMIT,umsg,Loginuser,0);
	if (onesys == ALLSYS) {
		...
		if (strncmp(Rmtname, Myname, SYSNSIZE) == SAME) {
			logent("SYSTEM STATUS", "CANNOT CALL SELF");
			cleanup(0);
		}
	}
...
#define TM_TAB_SIZE 30
char TM_table[TM_TAB_SIZE][NAMESIZE];
int TM_cnt = 0;
int TM_tab_size = TM_TAB_SIZE;
/*
