...
extern int omsg();
extern int TM_cnt;
extern int TM_tab_size;
extern char TM_table[][NAMESIZE];
...
/*
 *	Check for corrupted workfiles.  If a bad file is found
 *	exec uuclean to remove bad file and notify the owner
 */
		if (narg < 5)  {
			cp = strchr(Wfile,'.');
			sprintf(rqstr,"/usr/lib/uucp/uuclean -n0 -p%s",--cp);
			ret = shio(rqstr,NULL,NULL,Uucp);
			ASSERT(narg > 4,"ARG COUNT<5 - file removed",Wfile,ret);
			}
		...
		if (wrktype == RCVFILE) {
			...
			if (TM_cnt < TM_tab_size) {
				strcpy(TM_table[TM_cnt++],Dfile);
				dlogent(Dfile,"Temp file stored");
			}
			seqn++;
			setline(RCVFILE);
		}
...
process:
	...
	case HUP:
		 ...
		 * get work
		 */
		clscfile();  /*  close last workfile - if one was created */
