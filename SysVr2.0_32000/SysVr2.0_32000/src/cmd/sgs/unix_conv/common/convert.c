...
long genarc(), genobj(); /* routines for converting archives and object
			    files.  the routines return the number of bytes
			    in the converted file.  the converted file is
			    assumed to be in tmpfil.  a returned value of
			    0 implies that something has gotten screwed
			    up. */
...

{
	...
	if (argc != 3) {
		fprintf(stderr,"usage: convert [-5] infile outfile\n");
		exit(1);
	}
	...
	*/
	...
	switch (in_magic) {
		...
		case OARMAG:			/* archive input */
			if (!flag5)
			{
				fprintf(stderr, "convert: \"%s\": must specify -5 option to convert pre-5.0 archive\n", infilname);
				abort();
			}
			arcflag++;
	...
	while (bytes_out) {
		...
		bytes_out -= fwrite(buffer,1,bytes_in,outfil);
...

{
	...
	nm_newarc = tempnam("","conv");
	nm_member = tempnam("","conv");
	...
	*/
	...
	while (fread((char *)&old_fhdr, sizeof(old_fhdr), 1, arcfile) == 1)
	{
		...
		if (sprintf(hdr_buf, "%-16s%-12ld%-6u%-6u%-8o%-10ld%-2s",
			...
			sgetl(old_fhdr.arf_date), sgetl(old_fhdr.arf_uid),
			sgetl(old_fhdr.arf_gid), sgetl(old_fhdr.arf_mode),
