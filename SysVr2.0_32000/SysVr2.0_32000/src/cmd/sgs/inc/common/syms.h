...
#ifdef ns32000
	char			n_env;		/* symbol interpretation
						   environment */
	char			n_dummy;	/* unused */
#endif
};
...
union auxent
{
	struct
	{
		...
		union
		{
			struct
			{
				unsigned short x_callseq; /* calling sequence
							     information */
...
#ifdef ns32000
		short	x_fdummy;
		long	x_foff;		/* string table offset of filename */
#endif
	} 	x_file;
        struct
        {
...
#ifdef ns32000
#define x_tvndx x_fnend.x_befn.x_tvindx	/* tvndx needed to be unioned */
					/* can't be removed: mip code uses it */
#endif
