...
LOCAL Addrp pro1code(), epicode(), retval();
 ...
 * change. Functions changed: startproc, entrypt, dobss.
 */

static int FLAG_BF = 0;
...
/*	TEMPORARY FIX: When compiling with -g, a .bf symbol is put out for
	the main program. Also, .def symbols are put out for MAIN__ and
	progname. The assembler does not know which of these two names to
	assign the .bf symbol. FIX: Suppress printing of .def for progname.

	if (progname)
		prstab(nounder(XL, progname->extname),
		    strcat(buff, varstr(XL, progname->extname)), C_EXT,
		    T_INT | (DT_FCN << 4));
	comment for line above:	this type == int f();
*/
...
#if TARGET == NSC
if (nentry > 1)
   fatal("entry points not supported yet");
#endif
if(procclass == CLMAIN)
	{
...
q->vstg = STGAUTO;
q->ntempelt = nelt;
#if TARGET==PDP11 || TARGET==p || TARGET==NSC
