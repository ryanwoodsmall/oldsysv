...
static char Sccsid[] = "@(#)rjedisp.c	1.6";
...
struct jobdat {
	...
	char jd_place[128];	/* place from usr card */
...
register struct dsplog *dp;
{
	...
	if(qfull(dp->d_un.x.d_rdr)) {
		if((ip = deljobq(dp->d_un.x.d_rdr))!=(struct jqinfo *)0)
/*
...
doresp()
{
	...
	while(jobsout && fgets(buf,150,respfd) != NULL) {
		if((n=onrdr(buf)) >= 0) {
			if((ip = deljobq(n))!=(struct jqinfo *)0)
...
char *user;
{
	...
	if((fd=open("/etc/utmp",O_RDONLY)) >= 0) {
		while(read(fd,&utmp,sizeof(struct utmp)) == sizeof(struct utmp))
			if(strncmp(utmp.ut_user,user,8)==0) {
