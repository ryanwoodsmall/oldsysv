...
static char Sccsid[] = "@(#)rjexmit.c	1.4";
...
struct buffers *buffs[MAXDEVS];
struct buffers *malloc();
...
binit()
{
	...
	for(i=0; i<rdrs && i<MAXDEVS ;i++)
		if((buffs[i]=malloc(sizeof(struct buffers))) == (struct buffers *) NULL )
			return(0);
	return(1);
}
...
register int rdr;
{
	...
	if(fd == 0) {
		if(q->q_cnt > 0) {
			...
			if( (bufp=buffs[rdr]) != (struct buffers *) NULL ) {
				bufp->b_cnt = 0;
				bufp->b_index = 0;
				bufp->b_eof = FALSE;
			}
...
int fd, rdr;
{
	...
	if( msgflg || (bufp=buffs[rdr]) == (struct buffers *) NULL ) {
		for(;;) {
			if(read(fd,&ch,1) != 1) {
				*ptr++ = EOR;
				exitval = END;
		...
		for(;;) {
			cnt = 0;
			if( bufp->b_cnt )
				cnt = (int)bufp->b_buf[bufp->b_index];
			if( (cnt != 0 && cnt >= bufp->b_cnt) ||
			    (bufp->b_cnt == 0 && bufp->b_eof == FALSE) ) {
				if( bufp->b_eof == TRUE ) {
					error("premature end of file.\n", 0);
					return(END);
				}
				for( i = 0; i < bufp->b_cnt; i++)
					bufp->b_buf[i] = bufp->b_buf[bufp->b_index + i];
				bufp->b_index = 0;
				i = (BSZ - bufp->b_cnt) & ~(BSIZE - 1);
				j = read(fd, &bufp->b_buf[bufp->b_cnt], i);
				if( j < 0 ) {
			...
			bufp->b_index++;
			bufp->b_cnt--;
			ptr = mkrec(rcb, ISRCB, cnt, &bufp->b_buf[bufp->b_index], ptr);
			bufp->b_index += cnt;
			bufp->b_cnt -= cnt;
		}
	}
