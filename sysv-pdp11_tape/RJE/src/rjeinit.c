...
static char Sccsid[] = "@(#)rjeinit.c	1.2";
...
/* Logical unit name stuff for UNIX/370 */
...
*/
...
#ifdef	u370
	if(!r_boot) {
		if((acu = open(luname,O_RDONLY)) < 0)
			error("Can't open %s\n",luname,1);
		if(read(acu,lubuf,TAMNMSZ) <= 0)
			error("%s needs valid LU name\n",luname,1);
		close(acu);
		if((acu = open(RJECU,O_WRONLY)) < 0)
			error("Can't open %s\n",RJECU,1);
		if(write(acu,lubuf,strlen(lubuf)) < 0)
			error("Can't write %s (LU name)\n",RJECU,1);
		if(*phone) {
			if(write(acu,phone,strlen(phone)) < 0)
				error("Can't write %s (phone)\n",RJECU,1);
			fprintf(stderr,"Number loaded and %s closed\n",RJECU);
		}
		close(acu);
	}
#endif
	if(open(device,O_RDWR) != DEVFD) {
		if(errno == EACCES)
#ifdef	u370
			error("Can't open %s (No LU name)\n",device,0);
#else
			error("Can't open %s (permission denied)\n",device,0);
#endif
#ifdef	u370
		if(errno == EEXIST)
			error("Can't open %s (LU name not in table)\n",device,0);
		if(errno == EIO)
			error("Can't open %s (Connect failed)\n",device,0);
#endif
		if(errno == EIO)
			error("Can't open %s (physical device cannot be started)\n",device,0);
		if(errno == EUNATCH)
			error("Can't open %s (vpm device not attached to a physical device)\n",device,0);
		if(errno == ENXIO)
			error("Can't open %s (nonexistent vpm device)\n",device,0);
		if(errno == ENOMEM)
			error("Can't open %s (insufficient core available)\n",device,0);
		error("Can't open %s\n",device,0);
	}
#ifndef	u370
	if(*phone) {
