...
#define NEEDSNL 020		/* last record not ended with newline */
...
static char Sccsid[] = "@(#)rjerecv.c	1.4";
	...
	char **argv;
{
	...
	fsmin = atol(argv[1]);
	/* determine max file size; the default is no truncation,
	 * which corresponds to 1000000 bytes.
	 */
	if (argv[2][0] == '\0') max = 1000000;
		else max = atol(argv[2]) << 9;
...
fcnctl()
{
	...
	fcs = (getvpm() & 0xf) << 4;
	fcs |= (getvpm() & 0xf);
	if(((fcs & 0xfe) != 0xfe) || (devstate != 0xfe)) {
		mask = 0x80;
		for(n=0; n < MAXDEVS; n++) {
			if((mask & fcs) == 0) {
				devstate &= ~mask;
	...
	register int type;
{
	...
	if(dev->flag & XEOF) {
		if (dev->flag & NEEDSNL) write(dev->fd,"\n",1);
		close(dev->fd);
		dsplog.d_type = type;
