...
static char Sccsid[] = "@(#)rjestat.c	1.2";
...
/*
 *	qfiles returns the number of files in the "squeue"
 *	directory. (Number of queued files).
 */
...
char *dir;
{
	...
	for(;;) {
		...
		if(stat(dptr->d_name,&stb) < 0)
			error("Can't stat %s\n",dptr->d_name);
		oldest = (stb.st_ctime < oldest) ? stb.st_ctime : oldest;
		count++;
	}
	close(dfd);
	if(count)
		printf("%d file(s) queued since %s\n",count,ctime(&oldest));
}
