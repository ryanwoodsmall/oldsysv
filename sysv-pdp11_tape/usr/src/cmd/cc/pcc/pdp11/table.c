...
/* 	zzzcode uses these
		Z- putout (sp) or -(sp)
		Z4 putout -(sp) for doubles
		ZA logical OP for longs
		ZB putout a 'b' for byte
		ZC logical OP for longs
		ZD bit field mask
		ZF putout a branch
		ZH fix unsigned shifts
		ZI putout a branch
		ZL putout long constant
		ZM putout minus of value
		ZN set logical 1 or 0
		ZR struct assignment in a loop
		ZS struct assignment
		ZT truncate longs
		ZU same as AL but under U*
		ZV set upper word of the long
		ZW putout struct size
		ZY clear high byte for TUCHAR
		Z~ putout complement of value
*/
...
INCR,	INTAREG|INAREG|FOREFF,
	AWD,	TINT|TUNSIGNED|TPOINT|TCHAR,
	SONE,	TANY,
		NAREG,	RESC1,
		"F	movZB	AL,A1\n	incZB	AL\n",
...
#ifdef FORT
SCONV,	INTBREG|INBREG,
	SBREG,	TDOUBLE,
	SANY,	TFLOAT,
		0,	RLEFT,
		"",/* get rid of SCONV of double to float in BREG */
#endif
