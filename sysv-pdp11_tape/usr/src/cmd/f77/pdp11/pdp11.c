...
ftnint n;
{
/* this cheat depends on running on a machine with 2*short = long */
union { ftnint nlong; int nshort[2]; } cheat;
cheat.nlong = n;
if(type == TYLONG)
	fprintf(fp, "\t%d.;%d.\n", cheat.nshort[0], cheat.nshort[1]);
else
	fprintf(fp, "\t%ld.\n", cheat.nlong);
}
...
union { double xd; int xi[4]; } cheat;
cheat.xd = x;
k = (type==TYREAL ? 2 : 4);
fprintf(fp, "\t");
for(i = 0; i<k; ++i)
	fprintf(fp, "%d.%c", cheat.xi[i], (i==k-1 ? '\n' : ';') );
}
#endif
